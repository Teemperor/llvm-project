...
ClassType *AsClass(const CompilerType &ct) {
  return llvm::cast<ClassType>(
      static_cast<clike::Type *>(ct.GetOpaqueQualType()));
}
} // namespace

// A virtual base has no constant offset in the object; TypeSystemClike normally
...
TEST_F(ClangASTGeneratorVBaseTest, ComputeVBaseOffsetOffset) {
  ...
  builder.AddBaseClass(
      *b_cls, static_cast<clike::Type *>(a.GetOpaqueQualType()),

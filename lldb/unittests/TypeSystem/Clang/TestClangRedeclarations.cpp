...
TEST_F(TestClangRedeclarations, RedeclareCppClass) {
  ...
  CompilerType class_type = m_ast->CreateRecordType(
      ...
      TTK_Class, lldb::eLanguageTypeC_plus_plus);
...
TEST_F(TestClangRedeclarations, RedeclareCppTemplateClass) {
  ...
  args->args.push_back(TemplateArgument(m_ast->getASTContext().IntTy));
  args->names.push_back("T");

  ClassTemplateDecl *template_decl =
      m_ast->CreateClassTemplateDecl(m_ast->GetTranslationUnitDecl(), module_id,
                                     lldb::eAccessNone, "A", TTK_Struct, *args);
  ClassTemplateSpecializationDecl *fwd_decl =
      m_ast->CreateClassTemplateSpecializationDecl(
          m_ast->GetTranslationUnitDecl(), module_id, template_decl, TTK_Struct,
          *args);
...
TEST_F(TestClangRedeclarations, RedeclareObjCClass) {
  ...
  clang::ObjCMethodDecl *method = TypeSystemClang::AddMethodToObjCObjectType(
      objc_class, "-[A foo]", func_type, lldb::eAccessPublic, artificial,
      variadic, objc_direct);
  // Check that the method was created and is in the definition.
  ASSERT_NE(method, nullptr);
...
TEST_F(TestClangRedeclarations, NestedDecls) {
  ...
  CompilerType context_class = m_ast->CreateRecordType(
      ...
      lldb::eAccessNone, "A", TTK_Class, lldb::eLanguageTypeC_plus_plus);
  auto *fwd_decl =
  ...
  CompilerType nested_class = m_ast->CreateRecordType(
      fwd_decl, OptionalClangModuleID(), lldb::eAccessPublic, "A", TTK_Class,
  ...
  CompilerType nested_typedef = int_type.CreateTypedef(
      "t",
      CompilerDeclContext(m_ast.get(), static_cast<DeclContext *>(fwd_decl)),
      /*payload=*/0);
  ...
  ClassTemplateDecl *template_decl = m_ast->CreateClassTemplateDecl(
      fwd_decl, OptionalClangModuleID(), lldb::eAccessPublic, "A", TTK_Struct,
      args);
  EXPECT_EQ(template_decl->getDeclContext(), def);
  ClassTemplateSpecializationDecl *template_spec_decl =
      m_ast->CreateClassTemplateSpecializationDecl(
          fwd_decl, OptionalClangModuleID(), template_decl, TTK_Struct, args);

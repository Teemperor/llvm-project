...
class TestStack(TestBase):
    ...
    def test(self):
        ...
        self.expect("expr s_deque.size()", substrs=['(size_t) $0 = 3'])
        self.expect("expr s_deque.top().i", substrs=['(int) $1 = 4'])
        self.expect("expr s_deque.emplace(5)")
        self.expect("expr s_deque.top().i", substrs=['(int) $2 = 5'])

        # Test std::stack functionality with a std::vector.
        self.expect("expr s_vector.pop()")
        self.expect("expr s_vector.push({4})")
        self.expect("expr s_vector.size()", substrs=['(size_t) $3 = 3'])
        self.expect("expr s_vector.top().i", substrs=['(int) $4 = 4'])
        self.expect("expr s_vector.emplace(5)")
        self.expect("expr s_vector.top().i", substrs=['(int) $5 = 5'])

        # Test std::stack functionality with a std::list.
        self.expect("expr s_list.pop()")
        self.expect("expr s_list.push({4})")
        self.expect("expr s_list.size()", substrs=['(size_t) $6 = 3'])
        self.expect("expr s_list.top().i", substrs=['(int) $7 = 4'])
        self.expect("expr s_list.emplace(5)")
        self.expect("expr s_list.top().i", substrs=['(int) $8 = 5'])

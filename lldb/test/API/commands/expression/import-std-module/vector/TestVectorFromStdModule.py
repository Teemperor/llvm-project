...
class TestBasicVector(TestBase):
    ...
    def test(self):
        ...
        self.expect("expr a.front()", substrs=['(int) $7 = 3'])
        self.expect("expr a[1]", substrs=['(int) $8 = 2'])
        self.expect("expr a.back()", substrs=['(int) $9 = 1'])

        self.expect("expr *a.begin()", substrs=['(int) $10 = 3'])
        self.expect("expr *a.rbegin()", substrs=['(int) $11 = 1'])

        self.expect("expr a.pop_back()")
        self.expect("expr a.back()", substrs=['(int) $12 = 2'])
        self.expect("expr a.size()", substrs=['(size_t) $13 = 2'])

        self.expect("expr a.at(0)", substrs=['(int) $14 = 3'])

        self.expect("expr a.push_back(4)")
        self.expect("expr a.back()", substrs=['(int) $15 = 4'])
        self.expect("expr a.size()", substrs=['(size_t) $16 = 3'])

        self.expect("expr a.emplace_back(5)")
        self.expect("expr a.back()", substrs=['(int) $17 = 5'])
        self.expect("expr a.size()", substrs=['(size_t) $18 = 4'])

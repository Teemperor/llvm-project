...
class TestQueue(TestBase):
    ...
    def test(self):
        ...
        self.expect("expr q_deque.size()", substrs=['(size_t) $0 = 1'])
        self.expect("expr q_deque.front().i", substrs=['(int) $1 = 4'])
        self.expect("expr q_deque.back().i", substrs=['(int) $2 = 4'])
        self.expect("expr q_deque.empty()", substrs=['(bool) $3 = false'])
        self.expect("expr q_deque.pop()")
        self.expect("expr q_deque.emplace(5)")
        self.expect("expr q_deque.front().i", substrs=['(int) $4 = 5'])

        # Test std::queue functionality with a std::list.
        self.expect("expr q_list.pop()")
        self.expect("expr q_list.push({4})")
        self.expect("expr q_list.size()", substrs=['(size_t) $5 = 1'])
        self.expect("expr q_list.front().i", substrs=['(int) $6 = 4'])
        self.expect("expr q_list.back().i", substrs=['(int) $7 = 4'])
        self.expect("expr q_list.empty()", substrs=['(bool) $8 = false'])
        self.expect("expr q_list.pop()")
        self.expect("expr q_list.emplace(5)")
        self.expect("expr q_list.front().i", substrs=['(int) $9 = 5'])

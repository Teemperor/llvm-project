...
//
// Every thread debugs its own program, so the threads only share the debugger
// itself. If TEST_SHARED_MODULE is defined, all threads instead debug the same
// program (and therefore share LLDB's module for it) and every thread also
// launches its own process. See TestMultithreaded.py for the state of that
// mode.
...
#ifdef TEST_SHARED_MODULE
/// All threads debug the same program and each of them launches a process.
const bool shared_module = true;
#else
const bool shared_module = false;
#endif

/// The number of threads to use when all of them share one program. Otherwise
/// there is one thread per program that was passed to the test.
const unsigned shared_module_threads = 5;
...
void expect_string(const char *actual, const char *expected,
...
void expect_int(int64_t actual, int64_t expected, const std::string &what) {
  if (actual != expected)
...
void check_struct_type(SBType type) {
  ...
  expect(!type.IsPointerType() && !type.IsReferenceType() &&
             !type.IsArrayType() && !type.IsFunctionType() &&
             !type.IsTypedefType() && !type.IsVectorType() &&
             !type.IsAnonymousType() && !type.IsPolymorphicClass(),
         "'Data' was classified as some other kind of type");
  ...
  // Check the fields of the struct. Note that neither the base class nor the
  // static member are fields.
  ...
  expect(!first_field.IsBitfield(), "'i' should not be a bitfield");
  expect_int(first_field.GetBitfieldSizeInBits(), 0,
             "SBTypeMember::GetBitfieldSizeInBits");
  expect_int(first_field.GetOffsetInBits(), first_field.GetOffsetInBytes() * 8,
             "offset of first field 'i' in bits");
  ...
  // Check the types that can be derived from the struct type.
...
void check_struct_value(SBValue value, SBTarget &target) {
  ...
  if (shared_module) {
    // These only work while there is a process to read the value from.
    ...
    const char *summary = str.GetSummary();
    expect(summary && std::strstr(summary, "global struct"),
           "summary of 'g_data.str' doesn't contain the string value");
  }
}
...
std::string evaluate_and_check(SBDebugger &dbg, const std::string &program,
  ...
  try {
    ...
    if (shared_module) {
      // Stop in main. Note that a source regex breakpoint would race in
      // SourceManager instead of testing anything about expressions.
...
void test(SBDebugger &dbg, std::vector<std::string> args) {
  ...
  // Every thread debugs its own program unless all threads are supposed to
  // share a single module.
  const unsigned num_threads = shared_module
                                   ? shared_module_threads
                                   : static_cast<unsigned>(args.size());
  expect(num_threads > 1, "expected several programs to debug");
  ...
  for (unsigned i = 0; i < num_threads; ++i)
    threads.emplace_back([&, i]() {
      const std::string &program = shared_module ? args.at(0) : args.at(i);
      errors[i] = evaluate_and_check(dbg, program, barrier);
    });

...
class TestSwiftThickFunctionChildren(TestBase):
    ...
    @swiftTest
    def test(self):
        """A thick function is a record of a thin function pointer and a
        retainable context pointer. The function pointer's type is the
        reflection-internal `@convention(thin) () -> ()`, which demangles
        into a ThinFunctionType node."""
        self.build()

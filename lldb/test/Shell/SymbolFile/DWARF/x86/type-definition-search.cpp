...
// RUN: %lldb -O "settings set symbols.enable-typesystem-clike false" %t-n -o "target variable --ptr-depth 1 --show-types both_a both_b" -o exit | FileCheck %s
...
// RUN: %lldb -O "settings set symbols.enable-typesystem-clike false" %t-t -o "target variable --ptr-depth 1 --show-types both_a both_b" -o exit | FileCheck %s
...
// RUN: %lldb -O "settings set symbols.enable-typesystem-clike false" %t-tn -o "target variable --ptr-depth 1 --show-types both_a both_b" -o exit | FileCheck %s

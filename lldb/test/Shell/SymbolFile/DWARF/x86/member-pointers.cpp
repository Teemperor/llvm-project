...
// RUN: %lldb -O "settings set symbols.enable-typesystem-clike false" -f %t_linux.o -b -o "target variable s1 s2 m1 m2 v1 v2 v3 v4" | FileCheck --check-prefix=CHECK-GNU %s
//
...
// RUN: %lldb -O "settings set symbols.enable-typesystem-clike false" -f %t_win.exe -b -o "target variable s1 s2 m1 m2 v1 v2 v3 v4" | FileCheck --check-prefix=CHECK-MSVC %s
//

...
class TypeMatcher {
  ...
  /// True if this is just matching by the m_type_name string. False if
  /// m_type_name_regex should be used for matching.
  bool m_is_str;
  bool m_valid = true;
...
public:
  ...
  TypeMatcher(ConstString type_name) : m_type_name(type_name), m_is_str(true) {}
  /// Creates a matcher that accepts any type matching the given regex.
  TypeMatcher(RegularExpression regex)
      : m_type_name_regex(regex), m_is_str(false) {}
...
public:
  ...
  void Add(TypeMatcher name, const ValueSP &entry) {
    if (listener)
  ...
  bool Delete(const TypeMatcher &name) {
    std::lock_guard<std::recursive_mutex> guard(m_map_mutex);
    for (MapIterator iter = m_map.begin(); iter != m_map.end(); ++iter)
      if (iter->first.CreatedBySameMatchString(name)) {
        m_map.erase(iter);
        if (listener)
          listener->Changed();
        return true;
  ...
  bool Get(const TypeMatcher &name, ValueSP &entry) {
    std::lock_guard<std::recursive_mutex> guard(m_map_mutex);
    for (const auto &pos : m_map)
      if (pos.first.CreatedBySameMatchString(name)) {
        entry = pos.second;
        return true;
      }
    return false;

...
void IRExecutionUnit::GetStaticInitializers(
  ...
  for (llvm::Use &ctor_use : ctor_array->operands()) {
    ...
    if (!ctor_function) {
      LLDB_LOG(log, "global_ctor doesn't contain llvm::Function");
      continue;
    }
    ...
    for (JittedFunction &jitted_function : m_jitted_functions) {
      ...
      LLDB_LOG(log, "Calling function at address {0}.",
               jitted_function.m_remote_addr);
      break;
    }
    LLDB_LOG(log, "Couldn't find callable jitted function with name {0}.",
             ctor_function_name);
  }
}

...
clang::QualType ClangASTImporter::CopyType(clang::ASTContext *dst_ast,
  ...
  Minion::StdModuleScope std_scope(*minion_sp, dst_ast);
...
bool ClangASTImporter::CompleteTagDecl(clang::TagDecl *decl) {
  ...
  Minion::StdModuleScope std_scope(*minion_sp, &decl->getASTContext());
  if (minion_sp)
...
bool ClangASTImporter::CompleteAndFetchChildren(clang::QualType type) {
  ...
  if (const TagType *tag_type = type->getAs<TagType>()) {
    ...
    Minion::StdModuleScope std_scope(*minion_sp, &tag_decl->getASTContext());
...
llvm::Expected<Decl *> ClangASTImporter::Minion::ImportInternal(Decl *From) {
  if (m_std_handler) {
    ...
    if (D) {
      ...
      // decl. The decl the StdModuleHandler created has nothing to do with
  ...
  return ASTImporter::ImportInternal(From);
}

...
lldb::SBSymbolContextList SBModule::FindFunctions(const char *name,
  ...
  if (name && module_sp) {
    ...
    module_sp->FindFunctions(ConstString(name), CompilerDeclContext(), type, symbols_ok,
...
SBValueList SBModule::FindGlobalVariables(SBTarget &target, const char *name,
  ...
  if (name && module_sp) {
    ...
    module_sp->FindGlobalVariables(ConstString(name), CompilerDeclContext(), max_matches,
                                   variable_list);

...
static bool TemplateParameterAllowsValue(NamedDecl *param,
  ...
  if (auto *type_param = llvm::dyn_cast<TemplateTypeParmDecl>(param)) {
    ...
    assert(false && "Can't compare this TemplateParmDecl subclass");
...
/// For example, `<typename T>` allows the arguments `float`, but not for example
/// `bool, float` or `3` (as an integer parameter value).
static bool ClassTemplateAllowsToInstantiationArgs(
  ...
  TemplateParameterList &found = *class_template_decl->getTemplateParameters();
  ...
  size_t non_pack_params = found.size();
  for (size_t i = 0; i < found.size(); ++i) {
    NamedDecl *param = found.getParam(i);
  ...
  for (const auto pair : llvm::zip_first(instantiation_values.args, found)) {

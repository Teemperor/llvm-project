...
CompilerType clike_typesystem::CreateOpaqueObjCRecordType(Builder &builder,
                                                        llvm::StringRef name) {
  CompilerType record = builder.CreateRecordType(
      name, /*byte_size=*/1, /*is_cpp_class=*/false, /*is_union=*/false);
  if (auto *rt = llvm::dyn_cast_or_null<clike_typesystem::RecordType>(
          TypeSystemClike::GetClikeType(record.GetOpaqueQualType())))
    builder.SetRecordComplete(*rt);
  return record;
}

CompilerType clike_typesystem::RealizeObjCEncoding(Builder &builder,
                                                 llvm::StringRef &enc) {
...
void clike_typesystem::AddRuntimeObjCMethod(Builder &builder,
  ...
  // clike_typesystem::ObjCMethod's FunctionType does not carry (matching the
  // DWARF path -- see DWARFASTParserClike::CompleteObjCMethodsFromDWARF).

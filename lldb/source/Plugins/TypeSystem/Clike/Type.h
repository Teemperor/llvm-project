...
#ifndef LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_TYPE_H
#define LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_TYPE_H
...
/// A non-nullable reference to a Type.
///
/// Unlike a normal reference, this is copyable. The referenced type might be
/// located in a different Context than the Type that contains this reference.
class TypeRef {
public:
...
static_assert(sizeof(TypeRef) == sizeof(void *),
              "TypeRef is a commonly allocated object and should remain small");
...
struct Field {
  ...
  /// If this is a bitfield, then this stores the offset from the start of its
  /// storage unit.
  uint64_t byte_offset = 0;
  /// The width of this bitfield in bits. Zero means this is not a bitfield.
  uint32_t bitfield_bit_size = 0;
  /// The offset of its first bit within the storage unit at byte_offset.
  uint32_t bitfield_bit_offset = 0;

  bool IsBitfield() const { return bitfield_bit_size != 0; }
};

/// A base class in the generic language model.
///
/// For C++, this refers to a direct base class of class type.
/// For Objective-C, this models the interface's superclass.
struct BaseClass {
  /// The base class type.
  TypeRef type;
  /// Offset of the base class subobject from the start of the record.
  /// This is not set for virtual base classes where the offset is not fixed
  /// value.
  uint64_t byte_offset = 0;
...
public:
  ...
  /// The Context that owns this type.
  ...
  /// The namespace/class that contains this type.
  ...
  /// The unqualified name of this type. E.g., `string` or
  ...
  /// (e.g. an incomplete record, or a function type).
  virtual std::optional<uint64_t> GetByteSize() const { return std::nullopt; }

  /// An explicitly-specified alignment in bits such as types aligned with
  /// `alignas(128)`. If a type no explicitly-specified alignment, the type
  /// uses its natural alignment derived from its size.
  ...
  /// most of TypeSystemClike's query methods call this first. Sugar overrides it
...
#endif // LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_TYPE_H

...
// clike_typesystem one this file is all about; inside lldb_private the inner
// clike_typesystem::Type wins.
...
static void AppendNamespacePrefix(const clike_typesystem::Namespace *ns,
                                  std::string &out) {
  if (!ns)
    return;
...
static void AppendQualifiedNamespacePrefix(const clike_typesystem::Namespace *ns,
                                           std::string &out) {
  if (!ns)
    return;
...
static void AppendUnnamedTagNamespacePrefix(const clike_typesystem::Namespace *ns,
                                            std::string &out,
...
// enclosing classes.
static unsigned CountNamespaceDepth(const clike_typesystem::Namespace *ns) {
  unsigned depth = 0;
...
static std::string BuildUnnamedTagName(clike_typesystem::Type *t) {
...
static std::string BuildDisplayNameImpl(clike_typesystem::Type *t,
...
static void AppendAnonymousParentPrefix(clike_typesystem::Type *t,
                                        std::string &out) {
  auto *rec = llvm::dyn_cast<clike_typesystem::RecordType>(t);
  if (!rec)
    return;
  const clike_typesystem::RecordType *parent = rec->GetAnonymousParent();
  if (!parent)
    return;
  out += BuildDisplayNameImpl(const_cast<clike_typesystem::RecordType *>(parent));
  out += "::";
}
...
static std::string BuildPtrAuthQualifier(const clike_typesystem::PtrAuthType *pa) {
...
static std::string
BuildFloatArgName(const clike_typesystem::TemplateArgument &arg,
...
BuildTemplateArgName(const clike_typesystem::TemplateArgument &arg,
  ...
  if (value_type) {
    ...
    if (auto *enum_type = llvm::dyn_cast<clike_typesystem::EnumType>(value_type)) {
    ...
    if (llvm::isa<clike_typesystem::PointerType>(value_type) ||
        llvm::isa<clike_typesystem::ReferenceType>(value_type))
...
static std::string BuildTemplateArgList(clike_typesystem::RecordType *rec,
                                        bool hide_default_args,
                                        bool keep_inline_namespaces = false) {
  std::string args;
...
// integral.
static bool NeedsTemplateNameReconstruction(clike_typesystem::Type *t) {
  ...
  for (uint32_t i = 0, e = rec->GetNumTemplateArguments(); i != e; ++i) {
    ...
    if (llvm::isa_and_nonnull<clike_typesystem::EnumType>(value_type))
      return true;
    if (auto *builtin =
            llvm::dyn_cast_or_null<clike_typesystem::BuiltinType>(value_type))
      if (builtin->GetBuiltinKind() != clike_typesystem::BuiltinKind::Int)
        return true;
  }
  return false;
}
...
std::string clike_typesystem::BuildDisplayName(Type *t, bool hide_default_args,
                                             bool keep_inline_namespaces) {
  return BuildDisplayNameImpl(t, hide_default_args, keep_inline_namespaces);
}

std::string clike_typesystem::BuildFunctionName(FunctionType *fn,
                                              llvm::StringRef decl,
                                              bool keep_inline_namespaces) {
  return BuildFunctionNameImpl(fn, decl, keep_inline_namespaces);
}

std::string clike_typesystem::BuildCanonicalName(Type *t, bool base_only) {
  if (!t)
    return std::string();

...
// clike one this file is all about; inside lldb_private the inner
// clike::Type wins.
...
static void AppendNamespacePrefix(const clike::Namespace *ns,
                                  std::string &out) {
  if (!ns)
    return;
...
static void AppendQualifiedNamespacePrefix(const clike::Namespace *ns,
                                           std::string &out) {
  if (!ns)
    return;
...
static void AppendUnnamedTagNamespacePrefix(const clike::Namespace *ns,
                                            std::string &out,
...
// enclosing classes.
static unsigned CountNamespaceDepth(const clike::Namespace *ns) {
  unsigned depth = 0;
...
static std::string BuildUnnamedTagName(clike::Type *t) {
...
static std::string BuildDisplayNameImpl(clike::Type *t,
...
static void AppendAnonymousParentPrefix(clike::Type *t,
                                        std::string &out) {
  auto *rec = llvm::dyn_cast<clike::RecordType>(t);
  if (!rec)
    return;
  const clike::RecordType *parent = rec->GetAnonymousParent();
  if (!parent)
    return;
  out += BuildDisplayNameImpl(const_cast<clike::RecordType *>(parent));
  out += "::";
}
...
static std::string BuildPtrAuthQualifier(const clike::PtrAuthType *pa) {
...
static std::string BuildFunctionNameImpl(clike::FunctionType *fn,
                                     llvm::StringRef decl,
                                     bool keep_inline_namespaces = false) {
...
static std::string
BuildFloatArgName(const clike::TemplateArgument &arg,
                  clike::Type *value_type) {
...
// that raw spelling.
static std::string
BuildTemplateArgName(const clike::TemplateArgument &arg,
  ...
  if (value_type) {
    ...
    if (auto *enum_type = llvm::dyn_cast<clike::EnumType>(value_type)) {
    ...
    if (llvm::isa<clike::PointerType>(value_type) ||
        llvm::isa<clike::ReferenceType>(value_type))
...
static std::string BuildTemplateArgList(clike::RecordType *rec,
                                        bool hide_default_args,
                                        bool keep_inline_namespaces = false) {
  std::string args;
...
// integral.
static bool NeedsTemplateNameReconstruction(clike::Type *t) {
  ...
  for (uint32_t i = 0, e = rec->GetNumTemplateArguments(); i != e; ++i) {
    ...
    clike::Type *value_type =
        arg->type ? &arg->type->Get() : nullptr;
    if (llvm::isa_and_nonnull<clike::EnumType>(value_type))
      return true;
    if (auto *builtin =
            llvm::dyn_cast_or_null<clike::BuiltinType>(value_type))
      if (builtin->GetBuiltinKind() != clike::BuiltinKind::Int)
        return true;
  }
  return false;
}
...
static std::string BuildDisplayNameImpl(clike::Type *t,
  ...
  if (auto *cv = llvm::dyn_cast<CVQualifiedType>(t)) {
    ...
    if (llvm::isa<PointerType>(cv->GetUnderlyingType())) {
  ...
  if (auto *pa = llvm::dyn_cast<PtrAuthType>(t)) {
    ...
    if (llvm::isa<PointerType>(pa->GetUnderlyingType()))
...
std::string clike::BuildDisplayName(Type *t, bool hide_default_args,
                                             bool keep_inline_namespaces) {
  return BuildDisplayNameImpl(t, hide_default_args, keep_inline_namespaces);
}

std::string clike::BuildFunctionName(FunctionType *fn,
                                              llvm::StringRef decl,
                                              bool keep_inline_namespaces) {
  return BuildFunctionNameImpl(fn, decl, keep_inline_namespaces);
}

std::string clike::BuildCanonicalName(Type *t, bool base_only) {
  if (!t)
    return std::string();
  ...
  if (auto *cv = llvm::dyn_cast<CVQualifiedType>(t)) {
    ...
    while (true) {
      ...
      else if (auto *inner = llvm::dyn_cast<CVQualifiedType>(leaf))
        leaf = inner->GetUnderlyingType();
      else
        break;
    }
    const bool drop_qualifiers = llvm::isa<RecordType>(leaf) ||

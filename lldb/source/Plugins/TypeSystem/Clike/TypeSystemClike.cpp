...
ReadVirtualBaseOffset(TypeSystemClike &ts, clike::RecordType *derived,
                      const clike::BaseClass &base,
                      ValueObject *valobj) {
  if (!valobj)
    return std::nullopt;
...
static const clike::PtrAuthType *
FindPtrAuthType(const clike::Type *t) {
  while (t) {
    if (auto *pa = llvm::dyn_cast<clike::PtrAuthType>(t))
      return pa;
    if (auto *sugar = llvm::dyn_cast<clike::SugarType>(t)) {
...
CompilerType TypeSystemClike::GetCompilerType(clike::Type *type) {
  return CompilerType(weak_from_this(), type);
}

llvm::Expected<lldb::TypeSystemSP>
TypeSystemClike::Create(llvm::StringRef name, llvm::Triple triple) {
  llvm::Expected<clike::LanguageOpts> opts =
      clike::LanguageOpts::Create(std::move(triple));
  if (!opts)
...
ConstString TypeSystemClike::DeclGetName(void *opaque_decl) {
  // TypeSystemClike's CompilerDecls wrap a clike::Decl reference.
...
Scalar TypeSystemClike::DeclGetConstantValue(void *opaque_decl) {
  ...
  const auto *const *member =
      std::get_if<const clike::StaticDataMember *>(&decl->payload);
  if (!member || !(*member)->HasConstValue())
    return Scalar();
  clike::Type *desugared = (*member)->type.Get().Desugar();
...
ConstString TypeSystemClike::DeclContextGetName(void *opaque_decl_ctx) {
  // A TypeSystemClike CompilerDeclContext wraps a clike::Namespace (the
  // global namespace is a null opaque pointer, which is never a valid
  // CompilerDeclContext).
  if (!opaque_decl_ctx)
    return ConstString();
  auto read_lock = LockForRead();
  auto *ns = static_cast<const clike::Namespace *>(opaque_decl_ctx);
...
TypeSystemClike::DeclContextGetScopeQualifiedName(void *opaque_decl_ctx) {
  ...
  for (const clike::Namespace *cur = ns; cur; cur = cur->GetParent()) {
    if (cur->IsInline())
      continue;
...
TypeSystemClike::DeclContextGetCompilerContext(void *opaque_decl_ctx) {
  ...
  for (auto *ns = static_cast<const clike::Namespace *>(opaque_decl_ctx);
       ns; ns = ns->GetParent())
...
bool TypeSystemClike::DeclContextIsContainedInLookup(
  ...
  auto read_lock = LockForRead();
  auto *self = static_cast<const clike::Namespace *>(opaque_decl_ctx);
  auto *other =
      static_cast<const clike::Namespace *>(other_opaque_decl_ctx);
  for (const clike::Namespace *cur = other; cur;
       cur = cur->GetParent()) {
    if (cur == self)
      return true;
...
bool TypeSystemClike::IsArrayTypeImpl(const clike::Type *type,
                                    CompilerType *element_type, uint64_t *size,
                                    bool *is_incomplete) {
  if (element_type)
    element_type->Clear();
  ...
  auto *array = llvm::dyn_cast_or_null<clike::ArrayType>(
...
bool TypeSystemClike::IsCharType(opaque_compiler_type_t type) {
  ...
  // is recognized.
  auto *builtin =
      llvm::dyn_cast<clike::BuiltinType>(Desugar(t.get()));
...
bool TypeSystemClike::IsCompleteType(opaque_compiler_type_t type) {
  ...
  // via clike::Type::IsComplete(); only records/enums that were parsed
...
bool TypeSystemClike::IsFunctionType(opaque_compiler_type_t type) {
  SharedLockedType t = GetTypeForRead(type);
  return t && llvm::isa<clike::FunctionType>(Desugar(t.get()));
}
...
TypeSystemClike::GetNumberOfFunctionArguments(opaque_compiler_type_t type) {
  ...
  if (!t)
    return 0;
  if (auto *fn =
          llvm::dyn_cast<clike::FunctionType>(Desugar(t.get())))
...
bool TypeSystemClike::IsFunctionPointerType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return false;
  auto *ptr =
      llvm::dyn_cast<clike::PointerType>(Desugar(t.get()));
...
bool TypeSystemClike::IsMemberFunctionPointerType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return false;
  auto *mp =
      llvm::dyn_cast<clike::MemberPointerType>(Desugar(t.get()));
...
bool TypeSystemClike::IsBlockPointerType(
  ...
  if (!t)
    return false;
  auto *ptr =
      llvm::dyn_cast<clike::BlockPointerType>(Desugar(t.get()));
  if (!ptr)
    return false;
  ...
  if (function_pointer_type_ptr)
    *function_pointer_type_ptr =
        clike::Builder(*this).CreatePointerType(
...
bool TypeSystemClike::IsIntegerTypeImpl(const clike::Type *t,
                                      bool &is_signed) {
  is_signed = false;
  ...
  t = Desugar(t);
  if (!llvm::isa<clike::BuiltinType, clike::EnumType>(t))
    return false;
  // std::nullptr_t is a builtin with an unsigned (pointer-width) encoding but
  // is not an integer type.
  if (auto *bt = llvm::dyn_cast<clike::BuiltinType>(t))
    if (bt->GetBuiltinKind() == clike::BuiltinKind::NullPtr)
      return false;
...
bool TypeSystemClike::IsScopedEnumerationType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return false;
  if (auto *enum_type =
          llvm::dyn_cast<clike::EnumType>(Desugar(t.get())))
...
bool TypeSystemClike::IsPossibleDynamicType(opaque_compiler_type_t type,
  ...
  clike::Type *t = Desugar(tt.get());

  auto set_target = [&](clike::Type *pointee) {
  ...
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    clike::Type *pointee = ptr->GetPointeeType()->Desugar();
    if (llvm::isa<clike::ObjCInterfaceType>(pointee)) {
      if (check_objc) {
        set_target(pointee);
        return true;
      }
    ...
    if (check_objc && clike::IsOpaqueObjCObjectRecord(pointee)) {
      set_target(pointee);
      return true;
    }
  }
  ...
  // the C++ path.
  clike::Type *pointee = nullptr;
  bool is_reference = false;
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    pointee = ptr->GetPointeeType()->Desugar();
  } else if (auto *ref = llvm::dyn_cast<clike::ReferenceType>(t)) {
  ...
  if (pointee) {
    ...
    if (!is_reference) {
      if (auto *bt = llvm::dyn_cast<clike::BuiltinType>(pointee)) {
        if (bt->IsVoid()) {
          set_target(pointee);
          return true;
        }
    ...
    if (check_cplusplus && llvm::isa<clike::ClassType>(pointee)) {
...
bool TypeSystemClike::IsPointerType(opaque_compiler_type_t type,
  ...
  if (pointee_type)
    pointee_type->Clear();
  SharedLockedType tt = GetTypeForRead(type);
  auto *ptr = llvm::dyn_cast_or_null<clike::PointerType>(
...
bool TypeSystemClike::GetCompleteType(opaque_compiler_type_t type) {
  if (!type)
    return false;
  LockedType t = GetTypeForWrite(type);
  clike::Type *completed = CompleteTypeAssumingWriteLocked(t.get());
  return completed && completed->IsComplete();
}

clike::Type *TypeSystemClike::CompleteTypeAssumingWriteLocked(
    clike::Type *type) {
  if (!type)
    return nullptr;
...
bool TypeSystemClike::IsForcefullyCompleted(opaque_compiler_type_t type) {
  ...
  auto *record =
      llvm::dyn_cast_or_null<clike::RecordType>(Desugar(t.get()));
  if (!record)
    return false;
  clike::Type *completed = CompleteTypeAssumingWriteLocked(record);
  return !(completed && completed->IsComplete());
}

void TypeSystemClike::CompleteMemberFunctions(clike::Type *type) {
  if (!type)
    return;
...
void TypeSystemClike::CompleteMemberFunctionsAssumingWriteLocked(
  ...
  // GetCompleteType.
  auto *record =
      llvm::dyn_cast_or_null<clike::RecordType>(Desugar(type));
...
void TypeSystemClike::CompleteTemplateInstantiationForNameAssumingWriteLocked(
    clike::Type *t) {
  auto *rec = llvm::dyn_cast_or_null<clike::RecordType>(t);
  ...
  for (uint32_t i = 0, e = rec->GetNumTemplateArguments(); i != e; ++i) {
    const clike::TemplateArgument *arg =
...
ConstString TypeSystemClike::GetTypeNameAssumingWriteLocked(
    clike::Type *t, bool BaseOnly) {
  if (!t)
    return ConstString();
  CompleteTemplateInstantiationForNameAssumingWriteLocked(t);
  return ConstString(clike::BuildCanonicalName(t, BaseOnly));
}
...
ConstString
TypeSystemClike::GetDisplayTypeNameAssumingWriteLocked(clike::Type *t) {
  if (!t)
    return ConstString();
  CompleteTemplateInstantiationForNameAssumingWriteLocked(t);
  return ConstString(clike::BuildDisplayName(t));
}
...
TypeSystemClike::GetTypeInfo(opaque_compiler_type_t type,
  ...
  if (pointee_or_element_compiler_type) {
    clike::Type *inner = nullptr;
    if (auto *array = llvm::dyn_cast<clike::ArrayType>(Desugar(t)))
      inner = array->GetElementType();
    else if (auto *ptr =
                 llvm::dyn_cast<clike::PointerType>(Desugar(t)))
      inner = ptr->GetPointeeType();
    else if (auto *ref =
                 llvm::dyn_cast<clike::ReferenceType>(Desugar(t)))
...
LanguageType TypeSystemClike::GetMinimumLanguage(opaque_compiler_type_t type) {
  ...
  SharedLockedType tt = GetTypeForRead(type);
  if (tt) {
    const clike::Type *t = Desugar(tt.get());
    ...
    if (auto *ref = llvm::dyn_cast<clike::ReferenceType>(t))
    ...
    if (llvm::isa<clike::ObjCInterfaceType>(t))
      return eLanguageTypeObjC;
    if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
      const clike::Type *pointee = Desugar(ptr->GetPointeeType());
      ...
      if (clike::IsObjCObjectType(pointee))
        return eLanguageTypeObjC;
      if (!llvm::isa_and_nonnull<clike::RecordType>(pointee))
        return eLanguageTypeC;
    } else if (!llvm::isa<clike::RecordType>(t)) {
...
TypeSystemClike::GetArrayElementType(opaque_compiler_type_t type,
  ...
  if (!t)
    return CompilerType();
  if (auto *array = llvm::dyn_cast<clike::ArrayType>(Desugar(t.get())))
...
CompilerType TypeSystemClike::GetArrayType(opaque_compiler_type_t type,
  ...
  // summary.
  return clike::Builder(*this).CreateArrayType(
...
void TypeSystemClike::ForEachEnumerator(
  ...
  for (const clike::Enumerator &enumerator :
...
size_t TypeSystemClike::GetNumMemberFunctions(opaque_compiler_type_t type) {
  ...
  clike::Type *bearer = GetObjCBaseClassBearingType(tt.get());
  if (auto *iface = llvm::dyn_cast<clike::ObjCInterfaceType>(bearer)) {
    CompleteMemberFunctionsAssumingWriteLocked(iface);
    return iface->GetNumObjCMethods();
  }
  if (auto *record = llvm::dyn_cast<clike::RecordType>(t)) {
...
TypeSystemClike::GetMemberFunctionAtIndex(opaque_compiler_type_t type,
  ...
  if (auto *iface = llvm::dyn_cast<clike::ObjCInterfaceType>(bearer)) {
    CompleteMemberFunctionsAssumingWriteLocked(iface);
    const clike::ObjCMethod *method = iface->GetObjCMethodAtIndex(idx);
    if (!method)
      return TypeMemberFunctionImpl();
  ...
  auto *record = llvm::dyn_cast<clike::RecordType>(t);
  if (!record)
    return TypeMemberFunctionImpl();
  CompleteMemberFunctionsAssumingWriteLocked(record);
  const clike::MemberFunction *method =
  ...
  switch (method->kind) {
  case clike::MemberFunctionKind::Constructor:
    kind = lldb::eMemberFunctionKindConstructor;
    break;
  case clike::MemberFunctionKind::Destructor:
    kind = lldb::eMemberFunctionKindDestructor;
    break;
  case clike::MemberFunctionKind::Method:
  ...
  CompilerDecl decl(this, const_cast<clike::Decl *>(
...
CompilerType TypeSystemClike::GetPointeeType(opaque_compiler_type_t type) {
  ...
  if (!tt)
    return CompilerType();
  const clike::Type *desugared = Desugar(tt.get());
  ...
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(desugared))
  ...
  if (auto *ref = llvm::dyn_cast<clike::ReferenceType>(desugared))
...
CompilerType TypeSystemClike::GetPointerType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return CompilerType();
  return clike::Builder(*this).CreatePointerType(
      GetCompilerType(t.get()));
}
...
TypeSystemClike::GetLValueReferenceType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return CompilerType();
  return clike::Builder(*this).CreateReferenceType(
...
TypeSystemClike::GetTypeForFormatters(opaque_compiler_type_t type) {
  ...
  auto strip_cv = [](clike::Type *t) -> clike::Type * {
    while (auto *cv = llvm::dyn_cast_or_null<clike::CVQualifiedType>(t))
      t = cv->GetUnderlyingType();
    return t;
  };

  clike::Type *t = strip_cv(tt.get());
  if (!t)
    return CompilerType();

  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    clike::Type *pointee = ptr->GetPointeeType();
    clike::Type *stripped = strip_cv(pointee);
    if (stripped != pointee)
      return clike::Builder(*this).CreatePointerType(
  ...
  if (auto *array = llvm::dyn_cast<clike::ArrayType>(t)) {
    clike::Type *element = array->GetElementType();
    clike::Type *stripped = strip_cv(element);
    if (stripped != element)
      return clike::Builder(*this).CreateArrayType(
...
GetObjCRuntimeInstanceByteSize(const clike::Type *t,
                              Process *process) {
  if (!process)
    return std::nullopt;
...
TypeSystemClike::GetBitSize(opaque_compiler_type_t type,
  ...
  const clike::Type *t = Desugar(tt.get());
  if (llvm::isa<clike::ObjCInterfaceType>(t) && exe_scope) {
  ...
  if (llvm::isa<clike::FunctionType>(t))
    return 0;
...
TypeSystemClike::CreateRuntimeObjCInterface(ConstString class_name,
  ...
  // clike nodes via Builder, touches this instance and needs the
  // lock.
  llvm::SmallVector<RuntimeObjCClassData, 4> chain =
  ...
  auto write_lock = LockForWrite();
  clike::Type *previous_iface = nullptr;
  for (const RuntimeObjCClassData &data : llvm::reverse(chain)) {
    ...
    auto *iface = llvm::cast<clike::ObjCInterfaceType>(
    ...
    for (const RuntimeObjCClassData::Ivar &ivar : data.ivars) {
      ...
      CompilerType ivar_type = clike::RealizeObjCEncoding(builder, enc);
...
CompilerType
TypeSystemClike::GetRuntimeCompletedObjCType(clike::Type *t,
  ...
  // risk").
  auto *objc = llvm::dyn_cast_or_null<clike::ObjCInterfaceType>(t);
  if (!objc)
    return CompilerType();
  ...
  auto *runtime_iface =
      llvm::cast<clike::ObjCInterfaceType>(scratch_t.get());
...
llvm::Expected<uint32_t> TypeSystemClike::GetNumChildrenImpl(
    clike::Type *type, bool omit_empty_base_classes,
    const ExecutionContext *exe_ctx) {
  if (!type)
    return 0;
  clike::Type *t = Desugar(type);
  ...
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    if (clike::Type *pointee = ptr->GetTransparentChildPointee())
    ...
    return clike::IsVoid(ptr->GetPointeeType()) ? 0 : 1;
  }
  if (auto *ref = llvm::dyn_cast<clike::ReferenceType>(t)) {
    if (clike::Type *pointee = ref->GetTransparentChildPointee())
      return GetNumChildrenImpl(pointee, omit_empty_base_classes, exe_ctx);
    return 1;
  }
  if (!t->IsAggregate())
  ...
  if (omit_empty_base_classes) {
    auto complete = [this](clike::Type *bt) {
      CompleteTypeAssumingWriteLocked(bt);
    };
    uint32_t non_empty_bases = 0;
    for (uint32_t i = 0; i < num_bases; ++i) {
      const clike::BaseClass *base = t->GetBaseClassAtIndex(i);
      if (base && clike::RecordType::HasFields(&base->type.Get(), complete))
        ++non_empty_bases;
    }
...
bool TypeSystemClike::IsPromotableIntegerTypeImpl(
  ...
  t = Desugar(t);
  if (auto *builtin = llvm::dyn_cast<clike::BuiltinType>(t))
    return builtin->IsPromotableInteger();
  // Unscoped enumerations also promote.
  if (auto *enum_type = llvm::dyn_cast<clike::EnumType>(t))
    return !enum_type->IsScoped();
  return false;
}
...
TypeSystemClike::GetPromotedIntegerType(opaque_compiler_type_t type) {
  ...
  if (auto *enum_type = llvm::dyn_cast<clike::EnumType>(
          Desugar(tt.get()))) {
    ...
    if (int_size) {
      ...
      for (const clike::Enumerator &e : enum_type->GetEnumerators()) {
        if (enum_signed) {
...
clike::Type *
TypeSystemClike::GetObjCBaseClassBearingType(clike::Type *type) {
  clike::Type *t = Desugar(type);
  ...
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    clike::Type *pointee = ptr->GetPointeeType();
    if (llvm::isa<clike::ObjCInterfaceType>(pointee->Desugar())) {
      CompleteTypeAssumingWriteLocked(pointee);
...
bool TypeSystemClike::IsAnonymousType(opaque_compiler_type_t type) {
  ...
  if (auto *record =
          llvm::dyn_cast<clike::RecordType>(Desugar(t.get())))
...
llvm::Expected<CompilerType> TypeSystemClike::GetChildCompilerTypeAtIndexImpl(
    clike::Type *type, ExecutionContext *exe_ctx, size_t idx,
    bool transparent_pointers, bool omit_empty_base_classes,
    bool ignore_array_bounds, std::string &child_name,
    uint32_t &child_byte_size, int32_t &child_byte_offset,
    uint32_t &child_bitfield_bit_size, uint32_t &child_bitfield_bit_offset,
  ...
  if (auto *array = llvm::dyn_cast<clike::ArrayType>(t)) {
    ...
    clike::Type *element_type = array->GetElementType();
  ...
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    ...
    if (llvm::isa<clike::ObjCInterfaceType>(Desugar(pointee)))
      CompleteTypeAssumingWriteLocked(pointee);
    ...
    CompleteTypeAssumingWriteLocked(pointee);
    clike::Type *desugared_pointee = Desugar(pointee);
    ...
    if (llvm::isa<clike::ObjCInterfaceType>(desugared_pointee) &&
        exe_ctx)
  ...
  if (auto *ref = llvm::dyn_cast<clike::ReferenceType>(t)) {
    clike::Type *pointee = ref->GetPointeeType();
  ...
  for (uint32_t i = 0; i < total_bases; ++i) {
    ...
    if (!base)
      continue;
    if (omit_empty_base_classes &&
        !clike::RecordType::HasFields(&base->type.Get(), complete))
      continue;
    if (visible_base_idx == idx) {
      ...
      if (base->is_virtual) {
        auto *derived_rec = llvm::dyn_cast<clike::RecordType>(t);
  ...
  if (llvm::isa<clike::ObjCInterfaceType>(t) && exe_ctx) {
...
llvm::Expected<uint32_t> TypeSystemClike::GetIndexOfChildWithNameImpl(
    clike::Type *type, llvm::StringRef name,
    bool omit_empty_base_classes) {
  if (!type)
    return llvm::createStringError("invalid type");
  CompleteTypeAssumingWriteLocked(type);
  ...
  if (llvm::isa<clike::PointerType, clike::ReferenceType>(t)) {
    if (clike::Type *pointee = t->GetNamedMemberPointee())
...
size_t TypeSystemClike::GetIndexOfChildMemberWithNameImpl(
  ...
  if (llvm::isa<clike::PointerType, clike::ReferenceType>(t)) {
    clike::Type *pointee = t->GetNamedMemberPointee();
    if (!pointee)
      return 0;
  ...
  uint32_t total_bases = t->GetNumBaseClasses();
  auto base_is_visible = [&](const clike::BaseClass *base) {
...
static bool DumpEnumValue(const clike::EnumType &enum_type, Stream &s,
  ...
  // bits seen so far.
  const std::vector<clike::Enumerator> &enumerators =
      enum_type.GetEnumerators();
  if (enumerators.empty())
    can_be_bitfield = false;
  for (const clike::Enumerator &enumerator : enumerators) {
...
void TypeSystemClike::AppendMemberDeclAssumingWriteLocked(
    Stream &s, clike::Type *field_type, llvm::StringRef name) {
  using namespace clike;
  std::string dims;
  clike::Type *cur = field_type;
...
static void AppendMemberFunctionDecl(Stream &s,
  ...
  if (fn) {
    s << clike::BuildFunctionName(fn, name);
  } else {
    s << name << "()";
  }
  if (m.is_const)
...
void TypeSystemClike::DumpTypeDescription(opaque_compiler_type_t type, Stream &s,
  ...
  if (llvm::isa<clike::TypedefType>(tt.get())) {
  ...
  if (auto *iface = llvm::dyn_cast<clike::ObjCInterfaceType>(t)) {
    CompleteTypeAssumingWriteLocked(t);
    ...
    if (const clike::BaseClass *super = iface->GetBaseClassAtIndex(0))
      s.Printf(" : %s",
    ...
    for (uint32_t i = 0, e = iface->GetNumFields(); i != e; ++i) {
      const clike::Field *field = iface->GetFieldAtIndex(i);
      if (!field)
        continue;
      s.PutCString("    ");
    ...
    for (uint32_t i = 0, e = iface->GetNumObjCMethods(); i != e; ++i) {
      if (const clike::ObjCMethod *m = iface->GetObjCMethodAtIndex(i))
  ...
  if (auto *record = llvm::dyn_cast<clike::RecordType>(t)) {
    ...
    for (uint32_t i = 0, e = record->GetNumFields(); i != e; ++i) {
      const clike::Field *field = record->GetFieldAtIndex(i);
      if (!field)
        continue;
      s.PutCString("    ");
      AppendMemberDeclAssumingWriteLocked(s, &field->type.Get(), field->name.GetName());
...
void TypeSystemClike::Dump(llvm::raw_ostream &output, llvm::StringRef filter,
  ...
  std::vector<CompilerType> records;
  {
    ...
    auto read_lock = LockForRead();
    m_context.ForEachRecordType([&](clike::RecordType *record) {
...
bool TypeSystemClike::IsRuntimeGeneratedType(opaque_compiler_type_t type) {
  ...
  SharedLockedType t = GetTypeForRead(type);
  return t && llvm::isa<clike::ObjCInterfaceType>(Desugar(t.get()));
}
...
unsigned TypeSystemClike::GetTypeQualifiers(opaque_compiler_type_t type) {
  ...
  if (!t)
    return 0;
  return clike::CVQualifiedType::GetCVRMask(t.get());
}
...
CompilerType TypeSystemClike::GetBuiltinTypeByName(ConstString name) {
  ...
  if (name_ref == "__int128_t")
    return GetCompilerType(
        m_context.GetBuiltinType(clike::BuiltinKind::Int128));
  if (name_ref == "__uint128_t")
    return GetCompilerType(
        m_context.GetBuiltinType(clike::BuiltinKind::UnsignedInt128));

  if (clike::BuiltinType *bt =
...
TypeSystemClike::GetBasicTypeFromASTAssumingWriteLocked(BasicType basic_type) {
  ...
  case eBasicTypeObjCClass: {
    ...
    CompilerType ptr = builder.CreatePointerType(
        clike::CreateOpaqueObjCRecordType(
  ...
  case eBasicTypeObjCSel: {
    clike::Builder builder(*this);
    CompilerType ptr = builder.CreatePointerType(
        clike::CreateOpaqueObjCRecordType(builder, "objc_selector"));
  ...
  std::optional<clike::BuiltinKind> kind =
      clike::KnownBuiltinTypes::KindForBasicType(basic_type);
  if (!kind)
    return CompilerType();
...
TypeSystemClike::GetBuiltinTypeForEncodingAndBitSize(Encoding encoding,
  ...
  auto read_lock = LockForRead();
  std::optional<clike::BuiltinKind> kind =
      clike::KnownBuiltinTypes::KindForEncodingAndBitSize(encoding,
                                                                   bit_size);
  if (!kind)
    return CompilerType();
  return GetCompilerType(m_context.GetBuiltinType(*kind));
...
bool TypeSystemClike::IsConst(opaque_compiler_type_t type) {
  ...
  if (!t)
    return false;
  if (auto *cv = llvm::dyn_cast<clike::CVQualifiedType>(t.get()))
    return cv->IsConst();
  return false;
}
...
uint32_t TypeSystemClike::IsHomogeneousAggregate(opaque_compiler_type_t type,
  ...
  if (!tt)
    return 0;
  auto *record = llvm::dyn_cast_or_null<clike::RecordType>(
      Desugar(tt.get()));
  if (!record)
    return 0;
  clike::Type *completed = CompleteTypeAssumingWriteLocked(record);
...
bool TypeSystemClike::IsPolymorphicClass(opaque_compiler_type_t type) {
  ...
  if (!tt)
    return false;
  clike::Type *t = CompleteTypeAssumingWriteLocked(tt.get());
...
bool TypeSystemClike::IsTypedefType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return false;
  return llvm::isa<clike::TypedefType>(
      clike::ElaboratedType::Strip(t.get()));
}
...
CompilerType TypeSystemClike::GetTypedefedType(opaque_compiler_type_t type) {
  ...
  if (!t)
    return CompilerType();
  if (auto *td = llvm::dyn_cast<clike::TypedefType>(
          clike::ElaboratedType::Strip(t.get())))
...
bool TypeSystemClike::IsVectorType(opaque_compiler_type_t type,
  ...
  if (!tt)
    return false;
  const clike::Type *t = Desugar(tt.get());
  auto *array = llvm::dyn_cast<clike::ArrayType>(t);
...
clike::Type *
TypeSystemClike::GetFullyUnqualifiedTypeImpl(clike::Type *t) {
  ...
  while (auto *cv = llvm::dyn_cast<clike::CVQualifiedType>(t))
    t = cv->GetUnderlyingType();

  clike::Builder builder(*this);
  if (auto *ptr = llvm::dyn_cast<clike::PointerType>(t)) {
    clike::Type *pointee = ptr->GetPointeeType();
    clike::Type *stripped = GetFullyUnqualifiedTypeImpl(pointee);
    if (stripped != pointee)
      return static_cast<clike::Type *>(
  ...
  if (auto *array = llvm::dyn_cast<clike::ArrayType>(t)) {
    clike::Type *element = array->GetElementType();
    clike::Type *stripped = GetFullyUnqualifiedTypeImpl(element);
    if (stripped != element)
      return static_cast<clike::Type *>(
          builder
...
CompilerType TypeSystemClike::GetNonReferenceType(opaque_compiler_type_t type) {
  ...
  // `int`.
  if (auto *ref =
          llvm::dyn_cast<clike::ReferenceType>(Desugar(tt.get())))
    return GetCompilerType(ref->GetPointeeType());
  return CompilerType(weak_from_this(), type);
}
...
bool TypeSystemClike::IsReferenceType(opaque_compiler_type_t type,
  ...
  if (is_rvalue)
    *is_rvalue = false;
  SharedLockedType tt = GetTypeForRead(type);
  auto *ref = llvm::dyn_cast_or_null<clike::ReferenceType>(
      tt ? Desugar(tt.get()) : nullptr);
  if (!ref)
    return false;
  if (pointee_type)
...
static clike::RecordType *
GetRecordForTemplateArgs(clike::Type *type) {
  return llvm::dyn_cast_or_null<clike::RecordType>(Desugar(type));
}
...
TypeSystemClike::GetTemplateArgumentKind(opaque_compiler_type_t type, size_t idx,
  ...
  CompleteTypeAssumingWriteLocked(t.get());
  if (auto *record = GetRecordForTemplateArgs(t.get()))
    if (const clike::TemplateArgument *arg =
...
TypeSystemClike::GetIntegralTemplateArgument(opaque_compiler_type_t type,
  ...
  if (arg->type)
    if (llvm::isa<clike::PointerType>(&arg->type->Get()) ||
        llvm::isa<clike::ReferenceType>(&arg->type->Get()))
      return std::nullopt;
...
TypeSystemClike::GetDirectNestedTypeWithName(opaque_compiler_type_t type,
  ...
  CompleteTypeAssumingWriteLocked(t.get());
  if (auto *record = GetRecordForTemplateArgs(t.get()))
    if (clike::Type *nested = record->GetNestedTypeWithName(name))

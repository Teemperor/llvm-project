...
TypeSystemClike::TypeSystemClike(llvm::StringRef name, llvm::Triple triple)
    : m_display_name(name.str()), m_triple(std::move(triple)),
      m_context(clike_typesystem::LanguageOpts(m_triple)) {}
...
lldb::TypeSystemSP TypeSystemClike::Create(llvm::StringRef name,
                                         llvm::Triple triple) {
  return std::make_shared<TypeSystemClike>(name, std::move(triple));
}

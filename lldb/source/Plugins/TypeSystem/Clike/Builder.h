...
#ifndef LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_BUILDER_H
#define LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_BUILDER_H
...
/// The mutating interface used to populate a TypeSystemClike (e.g. by the DWARF
...
/// TypeSystemClike today runs single-threaded, either because the caller already
/// holds the Module mutex (the DWARF parser) or because it is only reached from
/// the driver thread. \ref DWARFASTParserClike's kMemberResolutionPolicy is the
...
class Builder {
public:
  explicit Builder(TypeSystemClike &ts);
  ...
  /// after completion; see DWARFASTParserClike::CompleteMemberFunctionsFromDWARF).
...
#endif // LLDB_SOURCE_PLUGINS_TYPESYSTEM_CLIKE_BUILDER_H

...
CompilerType Builder::GetVoidType() {
  return m_ts.GetCompilerType(
      m_ts.m_context.GetBuiltinType(clike::BuiltinKind::Void));
}
...
bool Builder::AddParameter(CompilerType function_type,
  ...
  auto *func = llvm::dyn_cast_or_null<clike::FunctionType>(
      static_cast<clike::Type *>(function_type.GetOpaqueQualType()));
  if (!func)
    return false;
...
void Builder::AddMemberFunction(clike::ClassType &record,
  ...
  if (!type)
    return;
  clike::MemberFunction method(*type);
...
void Builder::AddStaticDataMember(clike::ClassType &record,
                                  llvm::StringRef name, clike::Type *type,
...
void Builder::AddField(clike::RecordType &record,
                       clike::Identifier name,
                       clike::Type *type, uint64_t byte_offset,
...
void Builder::AddBaseClass(clike::ClassType &record,
                           clike::Type *type, uint64_t byte_offset,
                           bool is_virtual,
...
void Builder::SetRecordPolymorphic(clike::ClassType &record) {
  m_ts.m_context.SetPolymorphic(record);
}

void Builder::SetObjCSuperClass(clike::ObjCInterfaceType &record,
                                clike::Type *superclass) {
  if (std::optional<TypeRef> super = ToTypeRef(superclass))
    m_ts.m_context.SetObjCSuperClass(record, *super);
}

void Builder::AddObjCMethod(clike::ObjCInterfaceType &record,
                            llvm::StringRef name, CompilerType function_type,
...
void Builder::AddEnumerator(clike::EnumType &enum_type,
                            clike::Identifier name, uint64_t value) {
  m_ts.m_context.AddEnumerator(enum_type, name, value);
}

void Builder::AddTemplateArgument(clike::ClassType &record,
                                  lldb::TemplateArgumentKind kind,
                                  clike::Type *type,
                                  uint64_t integral_value, bool is_default) {
  clike::TemplateArgument arg;
  arg.kind = kind;
...
void Builder::AddTemplateTemplateArgument(clike::ClassType &record,
                                          llvm::StringRef name,
                                          bool is_default) {
  clike::TemplateArgument arg;
...
Builder::GetNamespace(llvm::StringRef name, const clike::Namespace *parent,
                      bool is_inline) {
...
void Builder::SetDeclContext(CompilerType type,
  ...
  if (auto *t = static_cast<clike::Type *>(type.GetOpaqueQualType()))
    t->SetDeclContext(ns);
}
...
void Builder::AddNestedType(clike::RecordType &record,
                            clike::Identifier name,
                            clike::Type *type) {

...
/// A TypeSystem for C, C++ and Objective-C that does not rely on Clang's AST.
///
/// This is the skeleton of that plugin: it registers itself with the
/// PluginManager so the rest of LLDB can see it, but it hands out no instances
/// yet (CreateInstance returns null and both supported-language sets are
/// empty), and every TypeSystem entry point below is a stub returning an
/// empty or otherwise invalid result. The implementation is filled in by the
/// commits that follow.

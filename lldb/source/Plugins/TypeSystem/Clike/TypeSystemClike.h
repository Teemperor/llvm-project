...
public:
  ...
  TypeSystemClike(llvm::StringRef name, clike::LanguageOpts opts);
  ~TypeSystemClike() override;

  /// The type system for \p triple, or an error if language options cannot be
  /// derived for that target (see clike::LanguageOpts::Create).
  ...
  CompilerType GetCompilerType(clike::Type *type);
  ...
  CompilerType GetCompilerType(const clike::Type *type) {
    return GetCompilerType(const_cast<clike::Type *>(type));
  }
  ...
  /// query API (the expression-parser plugins and clike::Builder,
  ...
  static clike::Type *GetClikeType(lldb::opaque_compiler_type_t type) {
    return static_cast<clike::Type *>(type);
  }
  ...
  /// the node records its owner -- see clike::Type::
  ...
  /// (mutation happens only through clike::Builder, which requires
  ...
  using LockedType = LockedIn<clike::Type *>;
  using SharedLockedType = LockedIn<const clike::Type *>;
  /// Same idea, but for the opaque CompilerDecl pointers handed out by the
  /// DeclGet*/GetTypeForDecl family (see clike::Decl -- a variant
  ...
  using SharedLockedDecl = LockedIn<const clike::Decl *>;
  ...
  SharedLockedDecl GetDeclForRead(void *opaque_decl) const {
    return SharedLockedDecl(
        ...
        static_cast<const clike::Decl *>(opaque_decl));
  }
  ...
  void CompleteMemberFunctions(clike::Type *type);
  ...
  void CompleteTemplateInstantiationForName(clike::Type *type);
  ...
  /// clike::Type to redirect from in that case, only a name.
  CompilerType CreateRuntimeObjCInterface(ConstString class_name,
                                          Process &process,
                                          ObjCLanguageRuntime &runtime);

private:
  friend class clike::Builder;
  ...
  clike::Type *
  CompleteTypeAssumingWriteLocked(clike::Type *type);
  ...
  void CompleteMemberFunctionsAssumingWriteLocked(clike::Type *type);
  ...
  ConstString GetTypeNameAssumingWriteLocked(clike::Type *t,
                                             bool BaseOnly);
  ...
  ConstString
  GetDisplayTypeNameAssumingWriteLocked(clike::Type *t);

  /// Helper for DumpTypeDescription; assumes the write lock is already held.
  void AppendMemberDeclAssumingWriteLocked(Stream &s,
                                           clike::Type *field_type,
                                           llvm::StringRef name);

  bool IsArrayTypeImpl(const clike::Type *type,
                       CompilerType *element_type, uint64_t *size,
                       bool *is_incomplete);

  bool IsIntegerTypeImpl(const clike::Type *type, bool &is_signed);

  bool IsPromotableIntegerTypeImpl(const clike::Type *type);

  llvm::Expected<uint32_t>
  GetNumChildrenImpl(clike::Type *type, bool omit_empty_base_classes,
  ...
  llvm::Expected<uint32_t>
  GetIndexOfChildWithNameImpl(clike::Type *type,
                              llvm::StringRef name,
                              bool omit_empty_base_classes);
  ...
  /// interface `Foo`.
  clike::Type *
  GetObjCBaseClassBearingType(clike::Type *type);
  ...
  clike::Type *
  GetFullyUnqualifiedTypeImpl(clike::Type *type);
  ...
  CompilerType GetRuntimeCompletedObjCType(clike::Type *t,
                                           const ExecutionContext *exe_ctx);
  std::string m_display_name;
  llvm::Triple m_triple;
  clike::Context m_context;
  ...
  llvm::StringMap<clike::Type *> m_runtime_objc_types;

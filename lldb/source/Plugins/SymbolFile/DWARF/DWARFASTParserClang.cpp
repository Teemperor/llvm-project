...
void DWARFASTParserClang::ParseMemberAttributes(
    const DWARFDIE &die, const DWARFDIE &parent_die,
    CompilerType &class_clang_type, const LanguageType class_language,
    std::vector<int> &member_accessibilities, AccessType &default_accessibility,
    DelayedPropertyList &delayed_properties,
    ClangASTImporter::LayoutInfo &layout_info, BitfieldInfo &last_field_info) {
  ...
  for (uint32_t i = 0; i < num_attributes && !is_artificial; ++i) {
    const dw_attr_t attr = attributes.AttributeAtIndex(i);
    DWARFFormValue form_value;
    if (attributes.ExtractFormValueAtIndex(i, form_value)) {
      switch (attr) {
...
bool DWARFASTParserClang::ParseChildMembers(
  ...
  for (DWARFDIE die = parent_die.GetFirstChild(); die.IsValid();
    ...
    case DW_TAG_member:
    case DW_TAG_APPLE_property:
      ParseMemberAttributes(die, parent_die, class_clang_type, class_language,

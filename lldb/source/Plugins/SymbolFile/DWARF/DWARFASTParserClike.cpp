...
// clike_typesystem::Builder). The work is nevertheless structured as independent
...
TypeSP DWARFASTParserClike::ParseBaseType(const DWARFDIE &die) {
  ...
  // enumerated builtins.
  CompilerType compiler_type = clike_typesystem::Builder(m_ts).GetBuiltinType(
...
BuildDeclNamespace(const DWARFDIE &die, clike_typesystem::Builder &builder) {
...
/// a namespace.
static const clike_typesystem::Namespace *
BuildNamespaceForDIE(const DWARFDIE &ns_die, clike_typesystem::Builder &builder) {
  DWARFDIE resolved = ResolveNamespaceAliasDIE(ns_die);
  if (!resolved)
    return nullptr;
  const clike_typesystem::Namespace *parent =
...
DWARFASTParserClike::GetDeclContextForUIDFromDWARF(const DWARFDIE &die) {
  ...
  // The wrapped opaque pointer is the interned clike_typesystem::Namespace.
  clike_typesystem::Builder builder(m_ts);
  ...
  return CompilerDeclContext(&m_ts,
                             const_cast<clike_typesystem::Namespace *>(ns));
}
...
DWARFASTParserClike::GetDeclContextContainingUIDFromDWARF(const DWARFDIE &die) {
  ...
  // CompilerDeclContext.
  clike_typesystem::Builder builder(m_ts);
  const clike_typesystem::Namespace *ns = BuildDeclNamespace(die, builder);
  if (!ns)
    return CompilerDeclContext();
  return CompilerDeclContext(&m_ts,
                             const_cast<clike_typesystem::Namespace *>(ns));
...
TypeSP DWARFASTParserClike::ParseStructureType(const DWARFDIE &die) {
  ...
  Declaration decl = GetDIEDeclaration(die);
  // The lldb_private::Type record (unlike the clike_typesystem::Type above) is
...
TypeSP DWARFASTParserClike::ParseArrayType(const DWARFDIE &die) {
  ...
  bool is_vector = die.GetAttributeValueAsUnsigned(DW_AT_GNU_vector, 0) != 0;
  {
    ...
    if (auto *arr = llvm::dyn_cast_or_null<clike_typesystem::ArrayType>(
...
TypeSP DWARFASTParserClike::ParsePointerType(const DWARFDIE &die) {
  ...
  CompilerType pointer_type =
      is_block ? clike_typesystem::Builder(m_ts).CreateBlockPointerType(
                     pointee_type)
               : clike_typesystem::Builder(m_ts).CreatePointerType(pointee_type);
...
TypeSP DWARFASTParserClike::ParseReferenceType(const DWARFDIE &die) {
  ...
  CompilerType reference_type =
      clike_typesystem::Builder(m_ts).CreateReferenceType(pointee_type,
                                                        is_rvalue);
...
TypeSP DWARFASTParserClike::ParsePointerToMemberType(const DWARFDIE &die) {
  ...
  CompilerType member_pointer_type =
      clike_typesystem::Builder(m_ts).CreateMemberPointerType(
...
TypeSP DWARFASTParserClike::ParseEnum(const DWARFDIE &die) {
  ...
  CompilerType enum_type;
  {
    ...
    clike_typesystem::Type *clike_type =
        TypeSystemClike::GetClikeType(enum_type.GetOpaqueQualType());
    auto &enum_node = *llvm::cast<clike_typesystem::EnumType>(clike_type);
...
bool DWARFASTParserClike::CompleteTypeFromDWARF(
  ...
  if (!clike_type || !clike_type->IsAggregate())
    return false;
  auto *record = static_cast<clike_typesystem::RecordType *>(clike_type);
  ...
  // of the same record bails out here.
  {
    ...
    case llvm::dwarf::DW_CC_pass_by_value:
      ts.SetRecordArgPassingKind(
          *record, clike_typesystem::RecordType::ArgPassingKind::PassByValue);
      break;
    case llvm::dwarf::DW_CC_pass_by_reference:
      ts.SetRecordArgPassingKind(
          *record,
          clike_typesystem::RecordType::ArgPassingKind::CannotPassInRegs);
      break;
    default:
      break;
    }
  ...
  auto *clike_class = llvm::dyn_cast<clike_typesystem::ClassType>(record);
  // An Objective-C class stores its (single) superclass separately.
  auto *objc_interface = llvm::dyn_cast<clike_typesystem::ObjCInterfaceType>(record);
  ...
  std::vector<std::future<clike_typesystem::Type *>> resolved;
  ...
  std::vector<clike_typesystem::Type *> member_types(members.size());
  ...
  auto *padding_type = static_cast<clike_typesystem::Type *>(
...
void DWARFASTParserClike::CompleteMemberFunctionsFromDWARF(
  ...
  if (auto *objc_iface =
          llvm::dyn_cast<clike_typesystem::ObjCInterfaceType>(&record)) {
  ...
  for (DWARFDIE child : die.children()) {
    ...
    clike_typesystem::RefQualifier ref_qualifier =
        clike_typesystem::RefQualifier::None;
    if (child.GetAttributeValueAsUnsigned(DW_AT_rvalue_reference, 0))
      ref_qualifier = clike_typesystem::RefQualifier::RValue;
    else if (child.GetAttributeValueAsUnsigned(DW_AT_reference, 0))
      ref_qualifier = clike_typesystem::RefQualifier::LValue;
    ...
    if (name_ref.starts_with("~"))
      ...
      kind = clike_typesystem::MemberFunctionKind::Constructor;
...
void DWARFASTParserClike::CompleteObjCMethodsFromDWARF(
    clike_typesystem::ObjCInterfaceType &iface, const DWARFDIE &class_die,
    clike_typesystem::Builder &ts) {

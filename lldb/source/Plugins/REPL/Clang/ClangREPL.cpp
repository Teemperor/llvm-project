...
ConstString ClangREPL::GetSourceFileBasename() {
  return ConstString("c_repl_input");
}

const char *ClangREPL::GetAutoIndentCharacters() { return "  "; }

bool ClangREPL::SourceIsComplete(const std::string &source) {
  // There isn't a good way to know if the input source is complete or not,
  // so just say that every single REPL line is ready to be parsed.
  return !source.empty();
}

...
/// clike::Type hierarchy) into a Clang AST, so that LLDB's Clang-based
...
/// (Clang-AST-free) clike description, writing directly into a
...
/// originating clike type) so an expression's result type can be
...
class ClangASTGenerator {
  ...
  // ASTContext to map generated Clang types back to their clike origin.
...
public:
  ...
  /// clike::BaseClass::vbase_offset_offset and
  ...
  /// clike namespace \p clike_ns. Types whose declaration context is
  ...
  void RegisterNamespace(const clike::Namespace *clike_ns,
  ...
  clang::DeclContext *
  GetDeclContextForNamespace(const clike::Namespace *clike_ns);
  ...
  clang::NamespaceDecl *
  GetRegisteredNamespace(const clike::Namespace *clike_ns) const;

  /// The clike::Namespace a generator-created clang::NamespaceDecl
...
private:
  ...
  /// Complete a record's fields/bases from its clike description.
  ...
  /// clike description. The ivars are added as clang::ObjCIvarDecls
  ...
  /// operates on the bare clike::Type, not a ValueObject. Returns
  /// true if \p ts / \p iface were rebound.
  bool RedirectObjCInterfaceToRuntimeDefinition(
      TypeSystemClike *&ts, clike::ObjCInterfaceType *&iface);
  ...
  clang::CXXRecordDecl *BuildClassTemplateSpecializationDecl(
      TypeSystemClike &ts, clike::RecordType *rec,
  ...
  /// Map a clike builtin type to a Clang builtin QualType.
  clang::QualType GenerateBuiltin(clike::Type *clike_type);
  ...
  void BuildParams(clang::FunctionDecl *func, clang::QualType function_qt,
                   const clike::FunctionType *clike_fn = nullptr);
  ...
  clang::FunctionDecl *BuildFunction(clang::DeclarationName name,
                                     ...
                                     llvm::StringRef asm_label,
                                     bool is_extern_c = false,
                                     const clike::FunctionType *clike_fn =
                                         nullptr);
  ...
  /// clike::Type -> produced clang QualType (opaque pointer).
  llvm::DenseMap<clike::Type *, void *> m_generated;

  /// Named records generated so far, keyed by fully-qualified name. The same
  /// C/C++ record can be described by distinct clike::RecordType
  ...
  /// clike::Namespace -> the clang::NamespaceDecl the decl map created
  /// for it, so generated types are placed in the matching namespace.
  llvm::DenseMap<const clike::Namespace *, clang::NamespaceDecl *>
      m_namespaces;
  ...
  llvm::DenseMap<const clang::NamespaceDecl *, const clike::Namespace *>
      m_namespace_decls;

  /// Reverse map: opaque QualType (quals included) -> originating
  /// clike::Type, so cv-qualified variants stay distinct.
  llvm::DenseMap<void *, clike::Type *> m_reverse;

  /// The TypeSystemClike that actually owns each clike::Type reachable
  ...
  /// pointing at the same clike::Type node its owning module parsed,
  ...
  llvm::DenseMap<clike::Type *, TypeSystemClike *> m_type_owner;
  ...
  llvm::DenseMap<clike::Type *, CompilerType> m_cross_module_complete;

  /// Per-record bookkeeping for lazy completion and layout.
  struct RecordInfo {
    TypeSystemClike *ts = nullptr;
    clike::RecordType *clike_record = nullptr;
    ...
    clike::RecordType *layout_record = nullptr;
  ...
  struct ObjCInterfaceInfo {
    TypeSystemClike *ts = nullptr;
    clike::ObjCInterfaceType *clike_iface = nullptr;

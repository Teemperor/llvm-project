...
/// clike origin, so an expression's result type can be expressed as a
...
/// clike -> Clang). It relies on the generator's reverse map (Clang
/// type -> originating clike type) for everything the generator
...
public:
  ...
  /// clike type) is consulted; must outlive this converter.
  ...
  /// Map \p qt (a Clang type) back to its clike origin. Returns an
...
private:
  ...
  void AddObjCMethod(clike::ObjCInterfaceType &iface,

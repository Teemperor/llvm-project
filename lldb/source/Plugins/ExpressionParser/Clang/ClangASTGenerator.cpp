...
// TypeSystemClike does. See clike::Type::Desugar.
using ct::Desugar;
...
void ClangASTGenerator::DumpRecords(TypeSystemClike &ts,
  ...
  // that are already complete in the clike model. A record parsed
...
clang::CXXRecordDecl *ClangASTGenerator::BuildClassTemplateSpecializationDecl(
  ...
  // driven by clike::TemplateArgument. Type arguments become
...
void ClangASTGenerator::PopulateRecord(clang::RecordDecl *record_decl) {
  ...
  for (uint32_t i = 0; i < rec->GetNumMemberFunctions(); ++i) {
    ...
    if (method_qt.isNull())
      continue;
    // The clike FunctionType doesn't carry the method's cv-qualifiers

...
bool CppModuleConfiguration::SetOncePath::TrySet(std::string path) {
...
bool CppModuleConfiguration::analyzeFile(const FileSpec &f) {
  ...
  if (libcpp_regex.match(f.GetPath())) {
    ...
    dir.consume_back("/experimental");
    return m_std_inc.TrySet(dir.str());
  }
...
CppModuleConfiguration::CppModuleConfiguration(
  ...
  bool error = !std::all_of(support_files.begin(), support_files.end(),
                            std::bind(&CppModuleConfiguration::analyzeFile, this, std::placeholders::_1));
  ...
  if (!error && hasValidConfig()) {
    ...
    llvm::sys::path::append(resource_dir, GetClangResourceDir().GetPath(), "include");

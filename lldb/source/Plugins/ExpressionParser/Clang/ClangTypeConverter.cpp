...
CompilerType ClangTypeConverter::Convert(clang::QualType qt) {
  ...
  if (llvm::isa<clang::TypeOfExprType, clang::TypeOfType, clang::DecltypeType>(
    ...
    if (CompilerType inner = Convert(qt.getCanonicalType())) {
      ...
      if (!spelling.empty()) {
        return clike::Builder(m_target).CreateElaboratedType(
            spelling, inner);
      }
      return inner;
    }
  ...
  if (qt.hasLocalQualifiers()) {
    if (CompilerType inner = Convert(qt.getLocalUnqualifiedType()))
      return clike::Builder(m_target).CreateCVQualifiedType(
...
CompilerType ClangTypeConverter::ConvertRecord(const clang::RecordType *rt) {
  ...
  if (rd && rd->isCompleteDefinition()) {
    ...
    clike::Builder builder(m_target);
  ...
  CompilerType fwd_record = clike::Builder(m_target).CreateRecordType(
...
CompilerType ClangTypeConverter::ConvertTypedef(
  ...
  if (!underlying)
    return {};
  CompilerType result = clike::Builder(m_target).CreateTypedefType(
  ...
  if (!decl->getDeclContext()->isRecord())
    ...
    clike::Builder(m_target).SetUnqualifiedName(result,
                                                         decl->getName());
  return result;
}
...
CompilerType ClangTypeConverter::ConvertObjCObjectPointer(
  ...
  if (CompilerType pointee = Convert(objc_ptr->getPointeeType()))
    return clike::Builder(m_target).CreatePointerType(pointee);
  return {};
}
...
CompilerType ClangTypeConverter::ConvertDerived(clang::QualType qt) {
  ...
  if (qt->isReferenceType()) {
    if (CompilerType pointee = Convert(qt->getPointeeType()))
      return clike::Builder(m_target).CreateReferenceType(
    ...
    if (CompilerType pointee = Convert(bpt->getPointeeType()))
      return clike::Builder(m_target).CreateBlockPointerType(pointee);
    ...
    if (CompilerType element = Convert(cx->getElementType()))
      return clike::Builder(m_target).CreateComplexType(element);
    ...
    if (CompilerType element = Convert(vt->getElementType())) {
      CompilerType arr = clike::Builder(m_target).CreateArrayType(
          element, vt->getNumElements());
      if (auto *arr_type = llvm::dyn_cast_or_null<clike::ArrayType>(
    ...
    if (CompilerType element = Convert(at->getElementType())) {
      ...
      return clike::Builder(m_target).CreateArrayType(element,
                                                               num_elements);
    }
  }
  return {};
...
void ClangTypeConverter::FillObjCInterface(const clang::ObjCInterfaceDecl *def,
  ...
  auto *iface = llvm::dyn_cast_or_null<clike::ObjCInterfaceType>(
...
void ClangTypeConverter::AddObjCMethod(clike::ObjCInterfaceType &iface,

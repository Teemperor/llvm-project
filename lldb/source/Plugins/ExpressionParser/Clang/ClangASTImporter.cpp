...
void ClangASTImporter::SetDeclOrigin(const clang::Decl *decl,
  ...
  ASTContextMetadataSP context_md = GetContextMetadata(&decl->getASTContext());
  context_md->setOrigin(decl, DeclOrigin(&original_decl->getASTContext(),
                                         original_decl));
}
...
void ClangASTImporter::ASTImporterDelegate::Imported(clang::Decl *from,
  ...
  if (from_context_md) {
    ...
    if (origin.Valid()) {
      if (!to_context_md->hasOrigin(to) && origin.ctx != &to->getASTContext())
      ...
      LLDB_LOG(log,
               ...
               "(ASTContext*){3}", origin.decl, origin.ctx,

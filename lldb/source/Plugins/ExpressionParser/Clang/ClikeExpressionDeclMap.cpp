...
/// clang::ExternalASTSource proxy that forwards to a ClikeExpressionDeclMap.
/// Mirrors ClangASTSource's proxy: the parser's ASTContext owns the source, so
/// the state-holding decl map is kept separate.
namespace {
class ClikeASTSourceProxy : public clang::ExternalASTSource {
public:
  explicit ClikeASTSourceProxy(ClikeExpressionDeclMap &map) : m_map(map) {}
...
private:
  ClikeExpressionDeclMap &m_map;
};
} // namespace

ClikeExpressionDeclMap::ClikeExpressionDeclMap(
...
ClikeExpressionDeclMap::~ClikeExpressionDeclMap() = default;

ClangASTGenerator &ClikeExpressionDeclMap::GetGenerator() {
  ...
  if (!m_generator) {
    ...
    // target (TypeSystemClike has no cross-module ASTImporter).
    m_generator->SetTarget(m_target);
  }
  return *m_generator;
}

void ClikeExpressionDeclMap::InstallASTContext(clang::ASTContext &ast) {
  m_ast_context = &ast;
}

/// The scratch TypeSystemClike that owns result/persistent types.
static TypeSystemClike *GetScratchClike(Target *target) {
  if (!target)
    return nullptr;
  auto ts_or_err =
  ...
  return llvm::dyn_cast_or_null<TypeSystemClike>(ts_or_err->get());
}

bool ClikeExpressionDeclMap::WillParse(ExecutionContext &exe_ctx,
...
void ClikeExpressionDeclMap::DidParse() {
...
void ClikeExpressionDeclMap::InstallCodeGenerator(clang::ASTConsumer *code_gen) {
  m_code_gen = code_gen;
}

void ClikeExpressionDeclMap::InstallDiagnosticManager(
    DiagnosticManager &diag_manager) {
  m_diagnostics = &diag_manager;
}

llvm::IntrusiveRefCntPtr<clang::ExternalASTSource>
ClikeExpressionDeclMap::CreateProxy() {
  return llvm::makeIntrusiveRefCnt<ClikeASTSourceProxy>(*this);
}

CompilerType ClikeExpressionDeclMap::WrapType(clang::QualType qt) {
  if (qt.isNull())
    return {};
  TypeSystemClike *scratch = GetScratchClike(m_exe_ctx.GetTargetPtr());
  if (!scratch)
    return {};
...
void ClikeExpressionDeclMap::StartTranslationUnit() {
  if (!m_ast_context)
    return;
...
void ClikeExpressionDeclMap::CompleteType(clang::TagDecl *tag_decl) {
  if (m_ast_context)
    GetGenerator().CompleteRecord(tag_decl);
}

void ClikeExpressionDeclMap::CompleteType(clang::ObjCInterfaceDecl *iface_decl) {
  if (m_ast_context)
    GetGenerator().CompleteObjCInterface(iface_decl);
}

bool ClikeExpressionDeclMap::LayoutRecordType(
    const clang::RecordDecl *record, uint64_t &size, uint64_t &alignment,
    llvm::DenseMap<const clang::FieldDecl *, uint64_t> &field_offsets,
...
clang::NamedDecl *ClikeExpressionDeclMap::CreateLocalVarsNamespace(
...
bool ClikeExpressionDeclMap::FindExternalVisibleDecls(
    const clang::DeclContext *dc, clang::DeclarationName name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  if (!m_ast_context)
    return false;
...
bool ClikeExpressionDeclMap::LookupFunctions(
  ...
  for (const SymbolContext &sc : ordered) {
    ...
    if (!func_type)
      continue;
    CompilerType func_clike_type = func_type->GetFullCompilerType();
    if (!func_clike_type || !func_clike_type.GetTypeSystem<TypeSystemClike>())
      continue;

    if (!seen_signatures.insert(func_clike_type.GetTypeName().GetStringRef())
             .second)
      continue;
...
bool ClikeExpressionDeclMap::LookupSymbolFunction(
...
const Symbol *ClikeExpressionDeclMap::FindCallableSymbol(ConstString name) {
  Target *target = m_exe_ctx.GetTargetPtr();
  if (!target)
    return nullptr;
...
void ClikeExpressionDeclMap::BindFunctionDeclToSymbol(clang::FunctionDecl *fd,
                                                    const Symbol *symbol) {
...
bool ClikeExpressionDeclMap::LookupModuleFunctions(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Target *target = m_exe_ctx.GetTargetPtr();
  if (!target)
  ...
  TypeSystemClike *scratch = GetScratchClike(target);
  if (!scratch)
    return false;
  ...
  for (const CompilerDecl &cd : found) {
    ...
    // there); the reconstructed type is owned by the scratch TypeSystemClike.
    ClangTypeConverter converter(GetGenerator(), *scratch, &fn->getASTContext());
    CompilerType func_clike_type = converter.Convert(fn->getType());
    if (!func_clike_type)
      continue;
    ...
    clang::FunctionDecl *fd = GetGenerator().GenerateFunction(
        name.GetStringRef(), func_clike_type, /*asm_label=*/{},
...
bool ClikeExpressionDeclMap::LookupGlobalDataSymbol(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Target *target = m_exe_ctx.GetTargetPtr();
  if (!target)
...
bool ClikeExpressionDeclMap::LookupOperatorFunctions(
    const clang::DeclContext *dc, clang::DeclarationName name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls,
  ...
  for (const SymbolContext &sc : sc_list) {
    ...
    clang::FunctionDecl *fd =
        GetGenerator().GenerateFunction(name, func_clike_type, label);
    if (!fd)
      continue;
...
clang::DeclContext *
ClikeExpressionDeclMap::GetOperatorDeclContext(Function *function) {
  if (!function)
    return nullptr;
  CompilerDeclContext ctx = function->GetDeclContext();
  if (!ctx || !llvm::isa_and_nonnull<TypeSystemClike>(ctx.GetTypeSystem()))
    return nullptr;
  // A free operator declared directly at global scope has no enclosing
  // TypeSystemClike namespace; keep it at translation-unit scope.
  auto *clike_ns = static_cast<const clike_typesystem::Namespace *>(
      ctx.GetOpaqueDeclContext());
  if (!clike_ns)
    return nullptr;
  return GetGenerator().GetDeclContextForNamespace(clike_ns);
}

void ClikeExpressionDeclMap::LookUpLldbClass(
    clang::DeclarationName name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  CompilerType class_clike_type;
  ...
  if (m_ctx_obj) {
    ...
    if (!ctx_obj_ptr || status.Fail())
      return;
    class_clike_type = m_ctx_obj->GetCompilerType();
  } else {
    ...
    if (!this_type)
      return;
    class_clike_type = this_type->GetForwardCompilerType().GetPointeeType();
    ...
    if (ValueObjectSP this_valobj =
      ...
      if (captured) {
        ...
        if (captured_pointee)
          class_clike_type = captured_pointee;
      }
    }
  }

  if (!class_clike_type || !class_clike_type.GetTypeSystem<TypeSystemClike>())
    return;
  ...
  bool this_is_volatile = false;
  {
    ...
    unsigned quals = class_clike_type.GetTypeQualifiers();
    ...
    auto ts = class_clike_type.GetTypeSystem<TypeSystemClike>();
    while (auto *cv = llvm::dyn_cast<clike_typesystem::CVQualifiedType>(
               TypeSystemClike::GetClikeType(class_clike_type.GetOpaqueQualType()))) {
      if (!ts)
        break;
      class_clike_type = ts->GetCompilerType(cv->GetUnderlyingType());
    }
  }
  if (!class_clike_type)
    return;

  clang::QualType class_qt = GetGenerator().Generate(class_clike_type);
  if (class_qt.isNull())
    return;
...
void ClikeExpressionDeclMap::LookUpLldbObjCClass(
    clang::DeclarationName name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  CompilerType class_clike_type;
  ...
  if (m_ctx_obj) {
    ...
    // is its pointee.
    class_clike_type = self_clike_type.GetPointeeType();
  }
  if (!class_clike_type || !class_clike_type.GetTypeSystem<TypeSystemClike>())
    return;
...
bool ClikeExpressionDeclMap::LookupLocalVariable(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  ...
  for (size_t i = 0, e = vars->GetSize(); i != e; ++i) {
    ...
    if (!var_type)
      return false;
    CompilerType var_clike_type = var_type->GetFullCompilerType();
    if (!var_clike_type)
      return false;

    clang::QualType qt = GetGenerator().Generate(var_clike_type);
    if (qt.isNull()) {
      LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for {0}", name);
      return false;
    }
  ...
  if (ValueObjectSP capture = find_capture(name, frame)) {
    ...
    if (!capture_type || !capture_type.GetTypeSystem<TypeSystemClike>())
      return false;
    clang::QualType qt = GetGenerator().Generate(capture_type);
    if (qt.isNull()) {
      LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for capture {0}",
               name);
      return false;
    }
    clang::QualType var_qt =
...
bool ClikeExpressionDeclMap::LookupGlobalVariable(
    const clang::DeclContext *dc, ConstString name, lldb::ModuleSP module,
    const CompilerDeclContext &scope,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  ...
  for (size_t i = 0, e = vars.GetSize(); i != e; ++i) {
    ...
    if (!scope.IsValid()) {
      ...
      if (var_ctx.IsValid()) {
        auto *ns = static_cast<const clike_typesystem::Namespace *>(
    ...
    if (!var_type)
      continue;
    CompilerType var_clike_type = var_type->GetFullCompilerType();
    // Only handle variables whose type is described by a TypeSystemClike; other
    // language plugins own their own decl-map path.
    if (!var_clike_type || !var_clike_type.GetTypeSystem<TypeSystemClike>())
      continue;

    clang::QualType qt = GetGenerator().Generate(var_clike_type);
    if (qt.isNull()) {
      LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for global {0}",
               name);
      continue;
    }
...
bool ClikeExpressionDeclMap::LookupType(
    const clang::DeclContext *dc, ConstString name, lldb::ModuleSP module,
    const CompilerDeclContext &scope,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  ...
  // TypeSystemClike path has no such helper, so do it here. A namespace-scoped
  ...
  if (!type_sp) {
    ...
    // TypeSystemClike::GetRuntimeCompletedObjCType, which does the same for a
    ...
    if (try_runtime) {
      if (lldb::ProcessSP process_sp = target->GetProcessSP()) {
        if (ObjCLanguageRuntime *runtime =
          ...
          if (TypeSystemClike *scratch = GetScratchClike(target)) {
  ...
  // Only handle types owned by a TypeSystemClike; other language plugins own
  // their own decl-map path.
  if (!type || !type.GetTypeSystem<TypeSystemClike>())
    return false;

  clang::QualType qt = GetGenerator().Generate(type);
  if (qt.isNull()) {
    LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for type {0}", name);
    return false;
  }
...
bool ClikeExpressionDeclMap::LookupNamespace(
  ...
  // TypeSystemClike-backed CompilerDeclContext.
  NamespaceMap map;
  ...
  auto add_from_module = [&](const lldb::ModuleSP &module_sp,
    ...
    // Only namespaces owned by a TypeSystemClike participate in this path.
    if (found.IsValid() &&
        llvm::isa_and_nonnull<TypeSystemClike>(found.GetTypeSystem()))
...
bool ClikeExpressionDeclMap::MaterializeNamespaceMap(
  ...
  clang::ASTContext &ast = *m_ast_context;
  if (clang::NamespaceDecl *nsd = GetGenerator().GetRegisteredNamespace(clike_ns)) {
    clang::Decl::castToDeclContext(nsd)->setHasExternalVisibleStorage(true);
  ...
  //
  // Parent it under the generator's decl context for clike_ns's actual parent
  ...
  clang::DeclContext *real_parent =
      GetGenerator().GetDeclContextForNamespace(clike_ns->GetParent());
  ...
  if (clang::NamespaceDecl *existing = CppModuleHandler::FindImportedNamespace(
    ...
    clang::Decl::castToDeclContext(existing)->setHasExternalVisibleStorage(
        true);
    GetGenerator().RegisterNamespace(clike_ns, existing);
  ...
  auto *nsd = clang::NamespaceDecl::Create(
      ...
      /*Nested=*/false);
  clang::Decl::castToDeclContext(nsd)->setHasExternalVisibleStorage(true);
  GetGenerator().RegisterNamespace(clike_ns, nsd);
...
bool ClikeExpressionDeclMap::EnsureGeneratorNamespaceMap(
  ...
  const clike_typesystem::Namespace *clike_ns =
  ...
  // `clike_ns`. FindNamespace matches by name within a parent scope, so walk the
  ...
  std::function<CompilerDeclContext(SymbolFile *,
      ...
      resolve = [&](SymbolFile *sf,
                    const clike_typesystem::Namespace *ns) -> CompilerDeclContext {
    if (!ns)
  ...
  for (const lldb::ModuleSP &module_sp : target->GetImages().Modules()) {
    ...
    if (!sf)
      continue;
    CompilerDeclContext found = resolve(sf, clike_ns);
    if (found.IsValid())
      map.emplace_back(module_sp, found);
  }
...
bool ClikeExpressionDeclMap::LookupInNamespace(
...
bool ClikeExpressionDeclMap::LookupInFrameNamespaces(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  StackFrame *frame = m_exe_ctx.GetFramePtr();
  if (!frame)
  ...
  // The frame's TypeSystemClike (owns the namespaces of the module the frame is
  ...
  auto *ts = llvm::dyn_cast_or_null<TypeSystemClike>(ts_or_err->get());
  if (!ts)
    return false;
  ...
  if (!frame_ctx ||
      !llvm::isa_and_nonnull<TypeSystemClike>(frame_ctx.GetTypeSystem()))
    return false;
  ...
  for (auto *ns = static_cast<const clike_typesystem::Namespace *>(
           frame_ctx.GetOpaqueDeclContext());
       ns; ns = ns->GetParent()) {
    CompilerDeclContext scope(ts, const_cast<clike_typesystem::Namespace *>(ns));
    if (!found_function &&
...
bool ClikeExpressionDeclMap::LookupInFrameClass(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  StackFrame *frame = m_exe_ctx.GetFramePtr();
  ...
  for (size_t i = 0, e = vars.GetSize(); i != e; ++i) {
    ...
    if (qt.isNull()) {
      LLDB_LOG(log,
               "ClikeEDM: couldn't generate a parser type for static member {0}",
               qualified_name);
      continue;
    }
...
bool ClikeExpressionDeclMap::LookupPersistentVariable(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  ...
  if (!pvar_type || !pvar_type.GetTypeSystem<TypeSystemClike>())
    return false;

  clang::QualType qt = GetGenerator().Generate(pvar_type);
  if (qt.isNull()) {
    LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for pvar {0}", name);
    return false;
  }
...
bool ClikeExpressionDeclMap::LookupPersistentType(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  if (!m_persistent_vars)
  ...
  if (!pvar_type || !*pvar_type || !pvar_type->GetTypeSystem<TypeSystemClike>())
    return false;
  ...
  // clike_typesystem CompilerType (which is context-independent) survived.
  clang::QualType qt = GetGenerator().Generate(*pvar_type);
  if (qt.isNull()) {
    LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for persistent "
                  "type {0}",
             name);
    return false;
  }
...
bool ClikeExpressionDeclMap::LookupRegister(
    const clang::DeclContext *dc, ConstString name,
    llvm::SmallVectorImpl<clang::NamedDecl *> &decls) {
  Log *log = GetLog(LLDBLog::Expressions);
  ...
  CompilerType clike_type = scratch->GetBuiltinTypeForEncodingAndBitSize(
      reg_info->encoding, reg_info->byte_size * 8);
  if (!clike_type) {
    LLDB_LOG(log, "ClikeEDM: couldn't get a builtin type for register {0}",
             reg_info->name);
    return false;
  }

  clang::QualType qt = GetGenerator().Generate(clike_type);
  if (qt.isNull()) {
    LLDB_LOG(log, "ClikeEDM: couldn't generate a parser type for register {0}",
             reg_info->name);
    return false;
  }
  ...
  LLDB_LOG(log, "ClikeEDM: Added register {0}", reg_info->name);
  return true;
}

bool ClikeExpressionDeclMap::AddPersistentVariable(const clang::NamedDecl *decl,
                                                 ConstString name,
                                                 TypeFromParser type,
                                                 bool is_result,
                                                 bool is_lvalue) {
  ...
  // `type` was produced by WrapType, so it is already a TypeSystemClike type
  // living in the scratch TypeSystemClike.
  if (!type) {
    LLDB_LOG(log, "ClikeEDM: persistent variable type couldn't be mapped to a "
                  "TypeSystemClike type");
    return false;
  }
  ...
  if (is_result && m_materializer) {
    ...
    if (!err.Success()) {
      LLDB_LOG(log, "ClikeEDM: couldn't add result variable: {0}",
               err.AsCString());
      return false;
    }
  ...
  LLDB_LOG(log, "ClikeEDM: created persistent variable {0} with flags {1:x}", name,
           var->m_flags);
...
bool ClikeExpressionDeclMap::AddValueToStruct(const clang::NamedDecl *decl,
                                            ConstString name,
...
bool ClikeExpressionDeclMap::DoStructLayout() {
...
bool ClikeExpressionDeclMap::GetStructInfo(uint32_t &num_elements, size_t &size,
                                         lldb::offset_t &alignment) {
  if (!m_struct_laid_out)
    return false;
...
bool ClikeExpressionDeclMap::GetStructElement(const clang::NamedDecl *&decl,
                                            llvm::Value *&value,
...
lldb::addr_t ClikeExpressionDeclMap::GetSymbolAddress(ConstString name,

...
class CodeComplete : public CodeCompleteConsumer {
  ...
  static bool CompletionBefore(const CompletionWithPriority &a,
                               const CompletionWithPriority &b) {
    // High priority results should come first.
    if (a.Priority != b.Priority)
      return a.Priority > b.Priority;

    // Identical priority, so just make sure it's a deterministic order.
    return a.completion.GetUniqueKey() < b.completion.GetUniqueKey();
  }
...
public:
  ...
  ~CodeComplete() override {
    ...
    // CompletionRequest.
    llvm::sort(m_completions, CompletionBefore);

    for (const CompletionWithPriority &C : m_completions)
      m_request.AddCompletion(C.completion.GetCompletion(),

...
void ClangUserExpression::ClangUserExpressionHelper::ResetDeclMap(
  ...
  // clike::Type nodes rather than clang::Decls, so the ASTImporter-
  // based decl map can't copy them. Use the TypeSystemClike-aware decl map, which
  // synthesizes the parser's clang AST from the clike description.
  if (ModuleList::GetGlobalModuleListProperties().GetEnableTypeSystemClike()) {
...
void ClangUserExpression::ClangUserExpressionHelper::CommitPersistentDecls() {
  ...
  // clike-backed CompilerType, keyed by name, in the persistent-type
  ...
  // way to make the type available again is to remember it as a clike

...
TypeFromUser ClangExpressionDeclMap::DeportType(TypeSystemClang &target,
  ...
  assert(&target == GetScratch(*m_target));
...
bool ClangExpressionDeclMap::AddPersistentVariable(const NamedDecl *decl,
  ...
  if (m_parser_vars->m_materializer && is_result) {
    ...
    auto *clang_ast_context = GetScratch(*target);
  ...
  TypeSystemClang *context = GetScratch(*target);
  if (!context)
    return false;
...
void ClangExpressionDeclMap::AddOneGenericVariable(NameSearchContext &context,
  ...
  TypeSystemClang *scratch_ast_context = GetScratch(*target);

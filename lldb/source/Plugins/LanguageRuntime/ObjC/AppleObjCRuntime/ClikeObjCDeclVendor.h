...
#ifndef LLDB_SOURCE_PLUGINS_LANGUAGERUNTIME_OBJC_APPLEOBJCRUNTIME_CLIKEOBJCDECLVENDOR_H
#define LLDB_SOURCE_PLUGINS_LANGUAGERUNTIME_OBJC_APPLEOBJCRUNTIME_CLIKEOBJCDECLVENDOR_H
...
/// The TypeSystemClike-backed counterpart to AppleObjCDeclVendor. Used by
/// AppleObjCRuntimeV2::GetDeclVendor() instead of AppleObjCDeclVendor when
/// symbols.enable-typesystem-clike is on, so that the ObjC runtime's
...
/// CompilerType-only fallback) get a TypeSystemClike CompilerType instead of
...
/// That is fine: when symbols.enable-typesystem-clike is on, ClikeExpressionDeclMap
...
/// TypeSystemClike::GetRuntimeCompletedObjCType, which only prefers the runtime
...
/// AppleObjCDeclVendor is a type-system mismatch against a TypeSystemClike value
...
/// TypeSystemClike::RealizeObjCEncoding). Making FindDecls() succeed here was
...
class ClikeObjCDeclVendor : public DeclVendor {
public:
  ClikeObjCDeclVendor(ObjCLanguageRuntime &runtime);

  static bool classof(const DeclVendor *vendor) {
    return vendor->GetKind() == eClikeObjCDeclVendor;
  }
...
#endif // LLDB_SOURCE_PLUGINS_LANGUAGERUNTIME_OBJC_APPLEOBJCRUNTIME_CLIKEOBJCDECLVENDOR_H

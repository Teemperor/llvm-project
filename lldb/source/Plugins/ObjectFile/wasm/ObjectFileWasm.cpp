...
/// Reads a byte from the data extractor.
static llvm::Optional<uint8_t> GetByte(DataExtractor &section_header_data,
                                       lldb::offset_t *offset_ptr) {
  lldb::offset_t initial_offset = *offset_ptr;
  uint8_t value = section_header_data.GetU8(offset_ptr);
  if (*offset_ptr == initial_offset)
    return llvm::None;
  return static_cast<uint8_t>(value);
}

static llvm::Optional<llvm::StringRef>
GetWasmString(DataExtractor &section_header_data, lldb::offset_t *offset_ptr) {
  ...
  llvm::Optional<uint64_t> len =
      section_header_data.GetULEB128(&offset, uint64_t(1) << 32);
  if (!len)
    return llvm::None;

  const uint8_t *bytes = section_header_data.PeekData(offset, *len);
  if (!bytes)
    return llvm::None;

  llvm::StringRef str(reinterpret_cast<const char *>(bytes), *len);
  offset += *len;
  *offset_ptr = offset;
  return str;
}
...
bool ObjectFileWasm::DecodeNextSection(lldb::offset_t *offset_ptr) {
  ...
  // - the actual contents.
  llvm::Optional<uint8_t> section_id = GetByte(section_header_data, &offset);
  if (!section_id)
    return false;

  llvm::Optional<uint64_t> payload_len =
      section_header_data.GetULEB128(&offset, uint64_t(1) << 32);
  if (!payload_len)
    return false;
  ...
  if (*section_id == llvm::wasm::WASM_SEC_CUSTOM) {
    ...
    llvm::Optional<llvm::StringRef> sect_name =
        GetWasmString(section_header_data, &offset);
    if (!sect_name)
      return false;

    uint32_t section_length = *payload_len - (offset - prev_offset);
    ...
    m_sect_infos.push_back(section_info{*offset_ptr + offset,
...
void ObjectFileWasm::CreateSections(SectionList &unified_section_list) {
  ...
  for (const section_info &sect_info : m_sect_infos) {
    ...
    if (llvm::wasm::WASM_SEC_CODE == sect_info.id) {
      ...
      vm_addr = IsInMemory() ? sect_info.offset & 0xffffffff : 0;
      vm_size = IsInMemory() ? sect_info.size : 0;
    }
...
llvm::Optional<FileSpec> ObjectFileWasm::GetExternalDebugInfoFileSpec() {
  ...
  for (const section_info &sect_info : m_sect_infos) {
    if (g_sect_name_external_debug_info == sect_info.name) {
      ...
      lldb::offset_t offset = 0;
      llvm::Optional<llvm::StringRef> symbols_url =
          GetWasmString(section_header_data, &offset);
      if (symbols_url)

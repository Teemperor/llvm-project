...
class StmtComparer {
  ...
  bool IsStmtEquivalent(const MemberExpr *E1, const MemberExpr *E2) {
    return IsStructurallyEquivalent(Context, E1->getFoundDecl(), E2->getFoundDecl());
  }
...
static bool IsStructurallyEquivalent(StructuralEquivalenceContext &Context,
                                     LabelDecl *D1,
                                     LabelDecl *D2) {
  // FIXME: Test and handle MS asm labels.
...
bool StructuralEquivalenceContext::CheckKindSpecificEquivalence(
  ...
  #define DECL(DERIVED, BASE) case Decl::Kind:: DERIVED : \

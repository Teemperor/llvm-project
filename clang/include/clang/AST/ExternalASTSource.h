...
public:
  ...
  void setGeneration(const GenerationCounter *C) {
    GenCounter = C;
  }

  const GenerationCounter *getGeneration() const {
    return GenCounter;
  }
...
public:
  ...
  T get(Owner O) {
    if (auto *LazyVal = Value.template dyn_cast<LazyData *>()) {
      if (LazyVal->LastGeneration != LazyVal->ExternalSource->getGeneration()->get()) {
        LazyVal->LastGeneration = LazyVal->ExternalSource->getGeneration()->get();
